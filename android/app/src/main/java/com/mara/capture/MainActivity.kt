...
class MainActivity : AppCompatActivity() {
    ...
    private fun здоровье() = фоном {
        ...
        listOf(
            ...
            // Местный сбой работу не хоронит (`JobFlow.послеСбоя`), и это
            // правильно — но без этой строки вечный повтор виден владельцу
            // только как «в очереди: 1», которое никогда не станет нулём.
            // Беда писалась в пяти местах и не читалась ни в одном.
            // Не «последняя с бедой»: `pending()` идёт `order by mtime`, и
            // новейшая с бедой — это обычно свежая запись, поймавшая разовое
            // «сети нет», а застрявшая как раз самая старая. Спрашиваем про
            // упорство, а не про порядок: застряло то, что повторяется.
            // Порог в три попытки — чтобы строка не кричала при нормальной
            // работе: одна попытка без вайфая беду уже записала.
    ...
    private fun мастер() = фоном {
        ...
        listOf(
            ...
            // Выше по стеку его ловит `фоном`, заменяя целиком на
            // «не собралось» — ровно тогда, когда владелец отчёт и собирает.
            "читается: " + (последняя?.let {
                runCatching { Device.open(this, it)?.use { true } == true }
                    .fold({ if (it) "да" else "нет" },
                          { "нет (${it.javaClass.simpleName})" })
            } ?: "-"),
