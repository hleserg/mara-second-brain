...
class MainActivity : AppCompatActivity() {
    ...
    private fun здоровье() = фоном {
        ...
        listOf(
            ...
            // `latest()` — последняя по времени, а застрявшая почти никогда
            // не последняя: одна свежая запись рядом — и строка снова врёт
            // «—». Спрашиваем очередь про беду, а не про порядок.
            "застряло: " + (q.pending().lastOrNull { it.error != null }
                ?.let { "${it.name}: ${it.error}" } ?: "—"),
    ...
    private fun мастер() = фоном {
        ...
        listOf(
            ...
            "читается: " + (последняя?.let {
                runCatching { Device.open(this, it) != null }
