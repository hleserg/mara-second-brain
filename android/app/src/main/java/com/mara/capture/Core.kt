...
object Дерево {
    /**
     * Все файлы дерева, от корня вглубь.
     *
     * Плоского списка мало. ACR раскладывает записи по
     * `[гггг]/[ММ]/[дд]/[номер телефона]/`, и в самой выбранной папке файлов
     * нет вовсе: владелец выбирает корень, в котором лежит только `2026`.
     * Обход достаёт и такие записи, и те, что штатный рекордер кладёт в
     * корень прямо.
     *
     * Дерево чужое, поэтому два потолка, и оба — на каталогах. `глубина` —
     * сколько уровней ниже корня разрешено раскрыть: записи ACR лежат на
     * пятом, восемь оставляют запас на лишнюю обёртку сверху. `каталогов` —
     * сколько каталогов вообще разрешено раскрыть; у SAF раскрытие каталога
     * стоит запроса к провайдеру, так что потолок этот не про память, а про
     * время.
     *
     * Файлы под потолок не попадают намеренно. Первая редакция считала все
     * узлы разом, и на архиве ACR за пару лет каталоги съедали потолок
     * целиком: обход возвращал ноль записей — ровно тот симптом, который эта
     * правка и чинит. Плоскую же папку такой потолок резал бы там, где до
     * правки она отдавалась целиком. Неизвестна тут форма дерева, а не число
     * файлов в одной папке.
     *
     * Обход в глубину, а не вширь, и это следствие того же счёта: вширь
     * потолок отрезает всё разом, потому что каталоги верхних уровней
     * раскрываются раньше, чем находится первый файл. Вглубь он отрезает
     * только хвост.
     *
     * Какой конец окажется хвостом, решаем мы, а не провайдер. Порядок
     * `listFiles` у SAF не оговорён ничем, и обход «как дали» на большом
     * архиве мог бы истратить потолок на 2019 год, ни разу не дойдя до
     * сегодняшнего звонка. Каталоги ACR названы датами, поэтому подпапки
     * раскрываются от старших имён к младшим: потолок тратится на свежее, а
     * при равных именах выдача хотя бы повторяема от прогона к прогону.
     *
     * Живёт здесь, а не в `Device`, затем, что так проверяется на JVM: сам
     * `DocumentFile` без Android не заводится.
     */
    fun <T> файлы(
        корень: T,
        папка: (T) -> Boolean,
        ...
        val найдено = mutableListOf<T>()
        val стопка = ArrayDeque<Pair<T, Int>>()
        стопка.addLast(корень to 0)
        var раскрыто = 0
        while (стопка.isNotEmpty() && раскрыто < каталогов) {
            val (узел, уровень) = стопка.removeLast()
            if (уровень >= глубина) continue
            // Потолок сторожим до `дети`, а не после: это запрос к
            // провайдеру, и упереться значит перестать спрашивать.
            раскрыто++
            // Имя спрашиваем по разу на каталог и держим рядом: у SAF это
            // запрос к провайдеру, а `sortBy { имя(it) }` дёргал бы его на
            // каждом сравнении.
            val подпапки = mutableListOf<Pair<String, T>>()
            for (это in дети(узел)) {
                if (папка(это)) подпапки.add(имя(это) to это) else найдено.add(это)
            }
            // По возрастанию имени, а снимаем с конца — значит вниз идём с
            // самого старшего: `09` раньше `08`, `2026` раньше `2025`.
            подпапки.sortBy { it.first }
            for ((_, это) in подпапки) стопка.addLast(это to уровень + 1)
        }
        return найдено
    }

    /**
     * То же дерево, но отобрано до того, что имеет смысл слать на сервер.
     *
     * Отбор здесь, а не в `Device`, по той же причине, что и сам обход: без
     * него `Device.folder` — три строки склейки, которые на JVM не проверить
     * ничем, а решение «этот файл уедет на сервер» слишком дорогое, чтобы
     * жить в непроверяемом месте.
     *
     * Возвращается не узел, а узел вместе с именем и размером: отбор их уже
     * спросил, и у SAF каждый такой вопрос — запрос к провайдеру. Пока
     * возвращались одни узлы, `Device.folder` спрашивал то же самое заново, и
     * плоская папка обходилась в шесть запросов на файл вместо четырёх.
     *
     * Порядок условий в отборе тоже не случайный: размер первым, имя после.
     * Пустой файл отсеивается, не заплатив за имя. Пустые — это записи,
     * которые рекордер ещё не дописал; `FileReady` их и так придержит, но
     * считать их найденными незачем.
     */
    fun <T> записи(
        корень: T,
        папка: (T) -> Boolean,
        ...
        имя: (T) -> String,
        размер: (T) -> Long,
    ): List<Triple<T, String, Long>> {
        val отобрано = mutableListOf<Triple<T, String, Long>>()
        for (это in файлы(корень, папка, дети, имя)) {
            val байт = размер(это)
            if (байт <= 0) continue
            val как = имя(это)
            if (Звук.похоже(как)) отобрано.add(Triple(это, как, байт))
        }
        return отобрано
    }
}
...
object Звук {
    /**
     * Расширения, по которым узнаём запись разговора.
     *
     * Отбор появился вместе с обходом вглубь и ровно из-за него. Пока
     * читался один каталог, отбирать было нечего: владелец сам показывал
     * папку ACR. Обход же разворачивает выбранную папку в целое поддерево, а
     * мастер говорит «выбери папку с записями» — покажут `/Documents`, и без
     * отбора на домашний сервер уехали бы сканы и PDF вместе со звонками.
     *
     * `mp4` намеренно нет: контейнер общий с видео. Полевого
     * подтверждения, что ACR им не пишет, у нас нет — есть одно имя
     * файла из отчёта владельца, и оно оканчивалось на `.m4a`. MIME
     * здесь не судья: `audio/mp4` тот же разбор даёт и для `.mp4`.
     * Соседний способ (медиатека) отбирает иначе — по приметам в пути
     * (`Device.mediaStore`), потому что там путь виден целиком, а у SAF
     * известно только имя листа.
     */
    private val РАСШИРЕНИЯ = setOf(
        "m4a", "mp3", "amr", "wav", "3gp", "3gpp", "ogg", "opus", "aac", "flac", "wma",
    )

    fun похоже(имя: String): Boolean =
        имя.substringAfterLast('.', "").lowercase() in РАСШИРЕНИЯ
}
...
object JobFlow {
     ...
     *
     * Состояние не меняется: попытка считается, беда записывается, работа
     * остаётся в очереди. Увести её отсюда в `FAILED` значило бы похоронить
     * разговор молча — `Store.pending()` работы в `FAILED` не отдаёт, а
     * поднимает их только `retryFailed()` при пересохранении токена. Сбои,
     * которые сюда долетают, почти все обратимы: отозванное разрешение,
     * отвалившийся SAF-грант, перемонтированная карта. Файл, который правда
     * исчез, сюда не попадает вовсе — его ловит `Device.open` и отдаёт null,
     * а `null`-ветки в `SyncWorker.шаг` и есть законный путь в `FAILED`.
     *
     * Цена вечного повтора — один неудачный `open` в четверть часа, и работа
     * при этом видна владельцу в «в очереди». Цена молчаливого `FAILED` —
     * потерянный разговор.
     */
    fun послеСбоя(job: Job, беда: String): Job =
        job.copy(attempts = job.attempts + 1, error = беда)
    ...
    /**
     * Отступить до следующего прогона, а не идти к следующей работе.
     *
     * 503 — единственный ответ, который просит именно подождать: сервер жив,
     * назвал причину («устройство уже льёт столько-то записей») и приложил
     * `Retry-After`. Следующая работа в очереди упрётся в тот же потолок, и
     * перебор очереди до конца — это мегабайты трафика ради того же отказа.
     *
     * Ноль сюда не входит нарочно: ноль — это «сети нет», прогон и так
     * обрывается, но повторять его через минуту бессмысленно, сеть вернёт
     * WorkManager по своему условию `NetworkType.CONNECTED`. Прочие
     * пятисотки — поломка на одной работе; ставить из-за неё всю очередь
     * значило бы дать одному битому файлу остановить остальные.
     */
    fun пауза(reply: ServerReply): Boolean = reply.code == 503
}
