...
object Дерево {
    /**
     * Всё, что не папка, от корня вглубь — уровень за уровнем.
     *
     ...
     * Обход вширь достаёт и такие записи, и те, что штатный рекордер кладёт
     * в корень прямо.
     *
     * Дерево чужое, поэтому два потолка. `глубина` — сколько уровней ниже
     * корня разрешено раскрыть: записи ACR лежат на пятом, восемь оставляют
     * запас на лишнюю обёртку сверху. `предел` — сколько записей вообще
     * разрешено осмотреть; у SAF каждая запись стоит отдельного запроса к
     * провайдеру, так что этот потолок не про память, а про время.
     *
    ...
    fun <T> файлы(
        ...
        var осмотрено = 0
        var уровень = listOf(корень)
        var шаг = 0
        while (уровень.isNotEmpty() && шаг < глубина) {
            val ниже = mutableListOf<T>()
            for (узел in уровень) {
                // Потолок сторожим и здесь: `дети` — это запрос к провайдеру,
                // и упереться в предел значит перестать спрашивать, а не
                // просто перестать складывать в ответ.
                if (осмотрено >= предел) break
                for (это in дети(узел)) {
                    if (осмотрено >= предел) break
                    осмотрено++
                    if (папка(это)) ниже.add(это) else найдено.add(это)
                }
            }
            уровень = ниже
            шаг++
