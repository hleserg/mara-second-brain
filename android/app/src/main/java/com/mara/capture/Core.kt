...
object Дерево {
    ...
    fun <T> файлы(
        ...
        while (стопка.isNotEmpty() && раскрыто < каталогов) {
            ...
            раскрыто++
            val подпапки = mutableListOf<T>()
            for (это in дети(узел)) {
                if (папка(это)) подпапки.add(это) else найдено.add(это)
            }
            // По возрастанию имени, а снимаем с конца — значит вниз идём с
            // самого старшего: `09` раньше `08`, `2026` раньше `2025`.
            подпапки.sortBy { имя(it) }
            for (это in подпапки) стопка.addLast(это to уровень + 1)
        }
        return найдено
    }
