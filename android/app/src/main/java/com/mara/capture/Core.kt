...
object Дерево {
     ...
     *
     * Пустые файлы — это записи, которые рекордер ещё не дописал; `FileReady`
     * их и так придержит, но считать их найденными незачем.
     */
    fun <T> записи(
        корень: T,
        папка: (T) -> Boolean,
        ...
        имя: (T) -> String,
        размер: (T) -> Long,
    ): List<T> = файлы(корень, папка, дети, имя)
        .filter { размер(it) > 0 && Звук.похоже(имя(it)) }
}
...
object Звук {
     ...
     *
     * `mp4` намеренно нет: контейнер общий с видео, а ACR им не пишет.
