...
#
# В коде — только те префиксы, которые ничего не выдают: `hleserg` — публичный
# GitHub-хэндл, `serg` — не системный логин. Машина, у которой домашний каталог
# назван системным логином владельца, живёт **вне** репозитория: репозиторий
# публичный (правило владельца от 2026-09-03), и держать логин в коде значит
# публиковать его. Дополнение берётся из `MARA_USAGE_HOSTS` или из файла
# `~/.config/mara/usage-hosts` — по паре `префикс=имя` на строку или через
# запятую.
#
# Без дополнения машина честно зовётся `unknown`. Это деградация, а не
# догадка: соврать «BetaPi» про чужой транскрипт хуже, чем признать незнание.
HOSTS = [("/home/hleserg", "BetaPi"), ("/Users/serg", "mac")]
ХОСТЫ_ФАЙЛ = "~/.config/mara/usage-hosts"


def хосты_ещё(окружение=None, файл=ХОСТЫ_ФАЙЛ):
    """Пары «префикс → машина» из-за пределов репозитория.

    Порядок: переменная окружения главнее файла — так разовый прогон можно
    поправить, не трогая настройку машины. Кривые пары пропускаются молча:
    панель использования не то место, ради которого стоит падать.
    """
    сырьё = (окружение if окружение is not None
             else os.environ.get("MARA_USAGE_HOSTS", ""))
    if not сырьё:
        try:
            сырьё = open(os.path.expanduser(файл), encoding="utf-8").read()
        except (OSError, UnicodeDecodeError):
            # Кодировка тут наравне с отсутствием файла: обещание «кривое
            # пропускаем молча» без неё держалось только на том, что файл
            # никто не сохранял в cp1251. `UnicodeDecodeError` — не `OSError`,
            # и уходил он не в `unknown`, а вверх через `host_of` и `parse`,
            # то есть ронял весь прогон, включая самопроверку.
            return []
    пары = []
    for кусок in сырьё.replace("\n", ",").split(","):
        префикс, _, имя = кусок.partition("=")
        if префикс.strip() and имя.strip():
            пары.append((префикс.strip(), имя.strip()))
    return пары
...
def host_of(cwd, project_key="", ещё=None):
    """Каталог проекта — запасной признак: у сессии в /tmp своего cwd нет, но имя
    папки транскрипта его помнит («-home-hleserg-…»).

    `ещё` — подмена дополнения для самопроверки. В работе оно читается один
    раз на процесс: `host_of` зовётся на каждую сессию, и ходить за ним в
    файл столько же раз значило бы платить тысячами `open` за три строки
    настройки.

    Самопроверка настоящий файл всё-таки читает — через `parse`, где `ещё`
    не передаётся. От настройки машины она при этом не зависит, но держится
    это не на подмене, а на порядке: `HOSTS` перебирается первым, и пары из
    кода дополнение перебить не может. Порядок закреплён утверждением.
    """
    global _ЕЩЁ
    if ещё is None:
        if _ЕЩЁ is None:
...
def self_check():
    ...
    # Машина, чей домашний каталог назван системным логином владельца, из кода
    # убрана: репозиторий публичный. Без дополнения она честно `unknown` — не
    # «BetaPi», не догадка.
    assert host_of("/home/кто-то/mara-second-brain", ещё=[]) == "unknown"
    assert host_of("/home/кто-то/mara", ещё=[("/home/кто-то", "doctor")]) == "doctor"
    assert host_of("", "-home-кто-то-mara", ещё=[("/home/кто-то", "doctor")]) == "doctor"
    # Разбор дополнения: запятая и перевод строки равноправны, пробелы по краям
    # не в счёт, кривая пара не роняет панель.
    assert хосты_ещё("/home/a=one") == [("/home/a", "one")]
    assert хосты_ещё(" /home/a=one , /home/b=two ") == [("/home/a", "one"), ("/home/b", "two")]
    assert хосты_ещё("/home/a=one\n/home/b=two") == [("/home/a", "one"), ("/home/b", "two")]
    assert хосты_ещё("мусор") == [] and хосты_ещё("=имя") == []
    assert хосты_ещё("", файл="/такого/нет") == []
    # Дальше — то, ради чего всё делалось, на своём файле и своей переменной.
    # Без этих трёх строк читалка файла могла не открывать файл вовсе, имя
    # переменной — быть любым, а порядок «код раньше дополнения» держаться
    # ничем: мутации показали, что четыре таких мутанта проходили самопроверку.
    проба = os.path.join(d, "usage-hosts-проба")
    open(проба, "w", encoding="utf-8").write("/home/кто-то=doctor\n")
    assert хосты_ещё("", файл=проба) == [("/home/кто-то", "doctor")], "файл не прочитан"
    было = os.environ.get("MARA_USAGE_HOSTS")
    os.environ["MARA_USAGE_HOSTS"] = "/home/другой=иной"
    try:
        assert хосты_ещё(файл=проба) == [("/home/другой", "иной")], "переменная не главнее файла"
    finally:
        if было is None:
            os.environ.pop("MARA_USAGE_HOSTS", None)
        else:
            os.environ["MARA_USAGE_HOSTS"] = было
    # Пары из кода перебиваются дополнением? Нет — и на этом стоит
    # независимость самопроверки от машины, на которой её запустили.
    assert host_of("/home/hleserg/x", ещё=[("/home/hleserg", "чужое")]) == "BetaPi"
    # Файл в чужой кодировке — не падение, а `unknown`: обещание §10.
    open(проба, "wb").write("/home/кто=вход\n".encode("cp1251"))
    assert хосты_ещё("", файл=проба) == [], "чужая кодировка уронила бы прогон"
