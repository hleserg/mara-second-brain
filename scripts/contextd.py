...
def metrics(con, root=None, vault=None):
    ...
    # По имени, а не по строке таблицы: `pair` не запрещает второе устройство
    # с тем же именем — перевыпуск токена так и делается, — а две строки с
    # одинаковым набором меток это не шум, а сломанная экспозиция: скрейп её
    # либо отвергает, либо молча берёт одну из строк. Свежесть берём самую
    # новую: имя молчит ровно тогда, когда молчат все его устройства.
