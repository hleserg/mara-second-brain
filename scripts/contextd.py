...
class Handler(BaseHTTPRequestHandler):
    ...
    def отлуп(self, code, текст, ещё=None):
        ...
        """
        # `ожидает_тела` — отказ на `Expect: 100-continue`: тела ещё нет ни
        # байта, сливать нечего, и попытка вычиталась бы до самого `timeout`,
        # держа поток ровно столько, сколько предел и заводился экономить.
        if not getattr(self, "ожидает_тела", False):
            try:
                слить(self.rfile, min(длина(self) or 0, СЛИВ), Никуда())
            except Exception:
                # Клиент объявил длину и замолчал: `слить` упирается в
                # `timeout` и бросает — а печатает у нас `say`, до которого
                # тогда не доходило. Отказ уходил без единой строки, ровно та
                # тишина, от которой чинили (#30). Сокет при этом жив, ответ
                # уйдёт.
                pass
