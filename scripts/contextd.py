...
def место_под_заливку(dev):
    ...
    Три — потому что штатно телефон льёт две: `mara-sync` и `mara-sync-once`
    (`Sync.kt:143`, `:163`) — разные unique-имена, WorkManager гоняет их
    независимо, а внутри воркера заливки идут по одной (`Sync.kt:59`).
    """
    with _замок:
    ...
    def do_POST(self):
        ...
        if p.path == "/v1/ingest/audio":
            ...
            if not место_под_заливку(dev):
                ...
                # (`Core.kt:136-146`) повторяет только `code == 0 or >= 500`,
                # всё прочее уходит в `else -> FAILED` — терминально, запись
                # пропадает совсем. 429 «слишком много» стоил бы владельцу
                # самого звонка ровно в тот момент, когда телефон торопится.
                # `Retry-After` отдаём для симметрии с путём 401, но телефон
                # его не читает вовсе: паузу держит WorkManager со своим
                # экспоненциальным откатом (`Sync.kt:147`).
