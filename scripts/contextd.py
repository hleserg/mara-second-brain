...
СЛИВ = 8 << 20
# Пауза, после которой слив на отказе по `Expect: 100-continue` считается
# законченным. Дождавшийся клиент не прислал ни байта, и ждать его до
# `timeout = 60` значит держать поток ровно столько, сколько предел и
# заводился экономить. Не дождавшийся льёт тело прямо сейчас, и его надо
# дочитать, иначе он увидит broken pipe вместо кода (#30) — две секунды
# терпимы к дрожанию сети и в тридцать раз дешевле полного ожидания.
ТИШИНА = 2.0
...
ЗАЛИВОК = 3                             # заливок аудио разом с устройства
...
_заливки = {}                           # устройство -> сколько льёт (#39)
...
def место_под_заливку(dev):
    """Занимает место под заливку аудио. False — устройство уже льёт ЗАЛИВОК.

    Предел на устройство, а не на сервер: заливка держит поток демона и
    `.part`-файл на диске платы всё время чтения тела, и без предела одно
    устройство занимает и то, и другое сколько угодно. Ключ — id устройства,
    а не имя: перевыпуск токена заводит вторую строку с тем же именем (#47),
    и лить может только живая.

    Три — потому что три и бывает. Внутри воркера заливки идут по одной
    (`Sync.kt:59`), но воркеров разом бывает три: периодический `mara-sync`
    (`Sync.kt:143`) и `mara-sync-once` (`:163`) — разные unique-имена, и
    WorkManager гоняет их независимо; а `REPLACE` у второго не останавливает
    отменённого — `isStopped` в приложении не проверяется вовсе, и старый
    дольёт своё, пока новый уже стартовал. Четвёртая получит 503 и вернётся.
    """
    with _замок:
        if _заливки.get(dev, 0) >= ЗАЛИВОК:
            return False
        _заливки[dev] = _заливки.get(dev, 0) + 1
        return True


def место_освободить(dev):
    """Возвращает место.

    pop на нуле, а не ноль значением: иначе словарь рос бы от каждого
    устройства, которое когда-либо лило, — та же гигиена, что у `_неудачи`.
    """
    with _замок:
        сколько = _заливки.get(dev, 0) - 1
        if сколько > 0:
            _заливки[dev] = сколько
        else:
            _заливки.pop(dev, None)
...
def читаемый(путь):
    ...
    Путь из одной латиницы в latin-1 кодируется, но из UTF-8 не декодируется —
    отдаём как есть. Байты, которые не UTF-8, декодировать нечем — тоже как
    есть: лог не то место, где можно падать.

    Два потолка, оба осознанные (круг 1 ревью PR #90, пп. 6–7):

    * функция не идемпотентна и «мохнатость» от настоящих символов не
      отличает: `/v1/Ã©` превратится в `/v1/é`. Отличить нечем — байты те же.
      Маршруты латинские, цена ошибки нулевая;
    * процентное кодирование не раскрывается. `unquote` пустил бы в строку
      лога `%0A`/`%0D`, которые `parse_request` сейчас отрезает, — то есть
      подмену строк в журнале через путь запроса. Журнал — journald, а не
      файл: `install/contextd.service` не задаёт `StandardOutput=`, так что
      stdout демона уходит туда. Первая редакция называла `reconcile.log` —
      это лог крона сверки, к `log_line` отношения он не имеет. Клиент, который
      кодирует (urllib, браузер, OkHttp), остаётся с непрозрачной строкой; это
      дешевле инъекции.
    """
    try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def отлуп(self, code, текст, ещё=None):
        ...
        Порядок — ответ, потом слив. Клиент узнаёт код сразу, а не после
        восьми мегабайт, и всё равно не остаётся без ответа: соединение
        закрывается только когда слив кончился. `say` сокет не закрывает,
        закрывает возврат из обработчика.
        """
        # `ожидает_тела` — отказ на `Expect: 100-continue`. Ждать `100` клиент
        # не обязан: заголовок спрашивает разрешения, но транспорт вправе
        # полить тело сразу. Поэтому сливаем и здесь — иначе закрытое с
        # непрочитанным телом соединение покажет ему broken pipe, ровно то, от
        # чего чинили #30. Разница одна: короткий таймаут. Дождавшийся не
        # пришлёт ни байта, и обычный слив упёрся бы в `timeout = 60`, держа
        # поток ровно столько, сколько предел и заводился экономить.
        #
        # Флаг одноразовый: снимается и здесь, и в `finally` у
        # `handle_expect_100`. Залипни он — короткий таймаут достался бы
        # обычному отказу на том же keep-alive соединении и оборвал бы слив на
        # первой же паузе медленного клиента, то есть вернул бы #30. Чтобы это
        # случилось, сломать надо оба снятия сразу.
        ждём = getattr(self, "ожидает_тела", False)
        self.ожидает_тела = False
        self.close_connection = True
        ответ = self.say(code, {"error": текст}, closing=True, ещё=ещё)
        if ждём:
            self.connection.settimeout(ТИШИНА)
        try:
            слить(self.rfile, min(длина(self) or 0, СЛИВ), Никуда())
        except Exception:
            # Клиент объявил длину и замолчал — или, прочитав код, закрылся
            # сам. `слить` упирается в таймаут либо в EOF и бросает; ответ к
            # этому времени уже ушёл, так что терять нечего.
            pass
        return ответ
    ...
    def аудио(self, con, p, n, dev):
        """Тело заливки. Вынесено из `do_POST` ради `finally` вокруг места.

        `dev` приходит готовым: место под заливку уже искали по нему, и второй
        поход в `devices` за тем же ответом здесь лишний.
        """
        eid = (urllib.parse.parse_qs(p.query).get("event") or [""])[0]
        ...
        row = con.execute("select kind from events where id=?",
                          (eid,)).fetchone()
        if row and not scope_ok(con, dev, row["kind"]):
            return self.отлуп(
                403, "устройству не разрешён вид «%s»" % row["kind"])
        code, out = ingest_audio(con, self.server.root, eid, self.rfile, n)
        if code < 400:
    ...
    def handle_expect_100(self):
        """Отказать до тела, если клиент спросил разрешения (#73, хвост Т3.3).

        Предел заливок отвечал 503 после того, как телефон выложил мегабайты:
        сервер обязан вычитать тело, иначе клиент видит broken pipe вместо
        кода. Платили оба — телефон трафиком, сервер потоком, занятым на весь
        слив (до СЛИВ = 8 МиБ). `Expect: 100-continue` снимает обе платы:
        клиент шлёт заголовки и ждёт, мы отказываем до первого байта.

        Место здесь **не занимается** нарочно. Между нашим `100` и телом
        проходит время, и занятое тут место пришлось бы возвращать в пути, где
        тела может не случиться вовсе. Гонка от этого остаётся: место может
        уйти между разрешением и `do_POST`, и тогда 503 придёт по-старому,
        после тела. Это ровно сегодняшнее поведение, то есть ухудшения нет —
        есть выигрыш там, где устройство занято уже на момент вопроса, а это
        и есть частый случай: четвёртая работа в очереди спрашивает, когда
        три предыдущие ещё льют.
        """
        p = urllib.parse.urlparse(self.path)
        if self.command != "POST" or p.path != "/v1/ingest/audio":
            return super().handle_expect_100()
        self.ожидает_тела = True
        try:
            if not self.authed(p.path):
                return False
            n = длина(self)
            if n is not None and n > MAX_BODY:
                self.отлуп(413, "тело больше %d МиБ" % (MAX_BODY >> 20))
                return False
            dev = device_of(self.con(), self.headers.get("Authorization"))
            with _замок:
                занято = _заливки.get(dev, 0) >= ЗАЛИВОК
            if занято:
                self.отлуп(503, "устройство уже льёт %d записей" % ЗАЛИВОК,
                           {"Retry-After": "60"})
                return False
        finally:
            self.ожидает_тела = False
    ...
    def do_POST(self):
        ...
        if p.path == "/v1/ingest/audio":
            ...
            # Место занимаем после 413: тот отказ диска и потока не держит.
            dev = device_of(con, self.headers.get("Authorization"))
            if not место_под_заливку(dev):
                # 503, а не 429, и это не вкусовщина. `JobFlow.next`
                # (`Core.kt:136-146`) продолжает работу на `code == 0 or
                # >= 500`; свои ветки есть у 200 и 409, а всё остальное —
                # 429 в том числе — уходит в `else -> FAILED`, терминально,
                # и запись пропадает совсем. 429 «слишком много» стоил бы
                # владельцу самого звонка ровно тогда, когда телефон спешит.
                # `Retry-After` отдаём для симметрии с путём 401, но телефон
                # его не читает вовсе, и отката по нему нет: `doWork` не
                # возвращает `Result.retry()` ни на одном пути, так что
                # `setBackoffCriteria` не срабатывает никогда. Повтор держат
                # периодика раз в 15 минут (`Sync.kt:143-145`) и `kick`.
                return self.отлуп(503, "устройство уже льёт %d записей"
                                  % ЗАЛИВОК, {"Retry-After": "60"})
            try:
                return self.аудио(con, p, n, dev)
            except ТаймаутТела as беда:
                ...
                # рожала бы «обрыв» на здоровой связи. И не всякий таймаут из
                # `аудио`, а только рождённый на чтении: `TimeoutError` из
                # записи блоба — поломка склада, и выдавать её за ушедшего
                # клиента значит соврать в диагнозе (нашёл Codex, круг 2).
                # Имя печатаем исходное: обёртка нужна была лишь для отбора.
                print(обрыв(беда.__cause__, self.client_address[0]), flush=True)
                ...
                # `handle_error` — и место утекало бы навсегда.
                место_освободить(dev)
...
class ТаймаутТела(TimeoutError):
    """Таймаут именно на чтении тела запроса, а не где-нибудь ещё.

    Нужен ровно для того, чтобы `do_POST` мог отличить ушедшего клиента от
    молчащего склада: `TimeoutError` из записи блоба на сетевой диск — это
    поломка сервера, и выдавать её за обрыв связи нельзя. Наследник
    `TimeoutError`, потому что выше по стеку его ловит и гасит базовый
    `handle_one_request`, и менять этот порядок правка не бралась.
    """
...
def finish_stored(con, root, event_id):
    ...
    Принимаются `new`, `stale` и `quarantined`: событие, брошенное на
    несошедшемся хеше, ещё может дойти — если файл не менялся, телефон
    пересчитает тот же sha и попадёт по дедупу в эту же строку.

    `quarantined` в этом списке — не запас на будущее, а единственный выход из
    карантина. Путь туда штатный и прописан самим ТЗ: нюх не узнал вид (§6.2),
    415 у телефона терминален (`android/.../Core.kt:280`), владелец расширяет
    нюх, `Store.kt:207 retryFailed()` поднимает работу заново — и тогда
    содержимое приходит второй раз и принимается. Без `quarantined` здесь
    событие после такой починки получало 200, блоб ложился в `calls/`, а
    состояние оставалось карантинным: манифеста нет, работы `asr` нет,
    расшифровки не будет никогда. Молчали при этом **все** проверки сверки
    (`запись_без_расшифровки` требует `stored|new|stale`, `блоб_без_манифеста`
    докладывает только файлы, не знакомые базе, `запись_не_долита` исключает
    `quarantined` явно), а находка `карантин` гасла ровно тем действием, о
    котором просит, — уборкой каталога. Перевод состояния — одним
...
def нюх(начало):
    ...
    Список закрытый: что здесь не названо, уходит в карантин. Мерка у списка
    одна и проверяемая: **всё, что грузит само приложение**
    (`android/.../Core.kt:171` — `РАСШИРЕНИЯ`, одиннадцать расширений), должно
    здесь узнаваться. Иначе приём отвергает то, что сам же и попросил прислать,
    а владельцу говорит «проверить формат записи в ACR» про нормальный формат.
    §7.2 ТЗ, на который первая редакция этого докстринга ссылалась, форматов не
    задаёт вовсе — задаёт задержку, битрейт, частоту и каталог; ссылка была
    неверной. Второе ограничение — ffmpeg, которым режет `call_asr.py`: принять
    то, чего он не прочтёт, значит перенести отказ в DLQ на шаг позже.
    Расширять список — правка одного этого места.
    """
    b = bytes(начало)
    if b[4:8] == b"ftyp":
    ...
    if len(b) >= 2 and b[:1] == b"\xff":
        сл = b[1]
        # ADTS — это `aac` из списка телефона: двенадцать бит синхры, дальше
        # слой `00`, который у MPEG audio «зарезервирован». Поэтому две ветки
        # ниже не спорят за одни и те же байты.
        if сл & 0xf6 == 0xf0:
            return "aac", "audio/aac"
        # MPEG audio — маской, а не перечислением байт. Перечисление
        # (`\xfb`, `\xf3`, `\xf2`) ловило три комбинации из двенадцати
        # осмысленных и роняло в карантин обычную запись: mp3 с защитой CRC
        # (`\xff\xfa`), MPEG 2.5 (`\xff\xe3`), Layer II (`\xff\xfd`).
        # Маска требует ровно того, что делает байт осмысленным: синхра `111`,
        # версия не «зарезервировано» (`01`), слой не «зарезервировано» (`00`).
        ...
        if сл & 0xe0 == 0xe0 and сл & 0x18 != 0x08 and сл & 0x06 != 0x00:
            return "mp3", "audio/mpeg"
    # ASF — это `wma` из того же списка. Шестнадцатибайтовый GUID заголовка;
    # восьми байт хватает, и они влезают в НЮХ.
    if b[:8] == bytes.fromhex("3026b2758e66cf11"):
...
def ingest_audio(con, root, event_id, поток, n=None):
    ...
    # Один и тот же момент на оба вызова `blob_path`. С `when=None` каждый
    # считал бы своё «сейчас», и у заливки, перешагнувшей полночь первого
    # числа, второй путь уезжал в каталог следующего месяца — которого ещё нет.
    # `os.replace` падал бы `FileNotFoundError` уже после приёма всех байт:
    # `finally` сносит `.part`, ответа телефону нет вовсе (`аудио()` ловит
    # только `ТаймаутТела`), телефон читает это как код 0 и повторяет. Не
    # потеря, но сорванная заливка раз в месяц — и до этой правки второго
    # вызова `blob_path` здесь не было вообще.
    когда = datetime.now(mi.TZ)
    path = mi.blob_path(root, want, ext, когда)
    ...
    try:
        ...
        # (`когда` один на оба вызова), меняется только расширение в имени,
        # поэтому replace остаётся внутри каталога и атомарным
        path = mi.blob_path(root, want, вид, когда)
        os.replace(tmp, path)
        # Тот же хеш когда-то не узнали и положили в карантин, а теперь узнали.
        # Копию оттуда сносим: это побайтово то же содержимое (хеш сверен
        # выше), диск оно занимает вторым разом, а находка `карантин` уровня
        # `error` иначе горела бы вечно — уборка каталога, о которой она
        # просит, владельцу уже не нужна. Имя строится от заявленного `ext`,
        # как и при укладке в карантин, поэтому совпадает.
        #
        # Без `try` тут гонка, и крючок к ней взводит сама находка: она велит
        # владельцу разобрать каталог, а разбор во время долетающего повтора
        # (или два повтора разом) уносит файл между `exists` и `unlink`.
        # `FileNotFoundError` вылетал бы из `ingest_audio` наружу — `аудио()`
        # ловит только `ТаймаутТела`, — телефон не получал бы ответа, а блоб
        # оставался бы опубликован в `calls/` без строки в `blobs`: находка
        # `блоб-без-манифеста` на пустом месте. Телефон лечится повтором, но
        # сводка владельцу — нет.
        try:
            os.unlink(карантин_путь(root, want, ext))
...
def bootstrap(con, vault=None):
    ...
    Дайджест — только доставленный. `sent_at` пишется безусловно, каким бы
    ни был исход отправки (`call_digest.py:163-166`: `mi.now_iso()` идёт в
    ту же строку, что и `state`), поэтому «последний по `sent_at`» — это
    последний записанный, а не последний полученный владельцем. Без
    фильтра клиент показал бы как отправленный текст, которого владелец не
    видел, и отличить не смог бы: `state` в выборку не входит.

    Фильтруем здесь, а не отдаём `state` клиенту: `bootstrap` описан как
    «что клиент должен знать, ничего больше не спрашивая», и недоставленный
    дайджест в таком ответе — ложь по умолчанию. Отдать состояние значило
    бы завести обязанность разбирать его в каждом клиенте (#65). Что
    недоставленные строки есть, владелец узнаёт из сверки, но читателей
    там два, а не один: `no-transport` и `not-private` называет
    `contextd_reconcile.дайджест_не_доставлен` (`:442` — фильтр ровно по
    этим двум), а `failed` до неё не доходит вовсе — `call_digest.py:169`
    поднимает `RuntimeError`, работа уходит в ретрай и при исчерпании
    попыток в DLQ, о которой говорит `dlq()`. Обе находки — в сводке
    8:00."""
    row = con.execute("select id,event_id,text,items_json,sent_at from digests "
                      "where state='sent' order by sent_at desc "
                      "limit 1").fetchone()
