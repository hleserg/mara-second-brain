    ...
    for подкаталог, вид, таблица, поля in ВИДЫ:
        ...
        for rel, fm, sha in карточки(vault, подкаталог):
            ...
            if row and проекция and row["id"] != проекция["object_id"]:
                ...
                # принадлежит другому: два точных ключа разошлись. Выбрать
                # между ними правило дедупликации (ТЗ §4.4) не даёт, а слить
                # автоматически — прямо запрещает. Обратный случай
                # (`source_id` убрали, native стал `vault:путь`) сюда же.
                итог["спорных"] += 1
