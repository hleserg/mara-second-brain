...
def run(con, vault=None, dry_run=False):
    ...
    for подкаталог, вид, таблица, поля in ВИДЫ:
        ...
        for rel, fm, sha in карточки(vault, подкаталог):
            ...
            # без файла. Поэтому спрашиваем ещё и проекцию по пути — вместе с
            # собственным ключом объекта, который за ней стоит, и его
            # заголовком: без ключа проекция говорит «здесь кто-то есть», но
            # не говорит, наш ли, а без заголовка — не отличает нашу карточку,
            # получившую `source_id`, от чужой, занявшей освободившийся путь.
            #
            # Соединение обычное, не `left`: у проекции, чью строку объекта
            # снесли руками, ответа нет вовсе, и это правильный ответ. При
            # `left join` вернулась бы пара из пустот, ключ не совпал бы ни с
            # чем, и карточка спорила бы вечно — вместо того чтобы просто
            # завестись заново. Мутант «join → left join» на этом и ловится.
            проекция = con.execute(
                "select o.id, o.source_native_id, o.title from projections p "
            ...
            # Так выглядят сразу несколько случаев, и точного признака, чтобы
            # их развести, нет: карточку переименовали на путь другой, на
            # месте удалённой завели новую, `source_id` из карточки убрали.
            # Правила, которое выбрало бы между ними, нет: ТЗ §4.3 прямо
            # требует доказательства, что это одно событие, а его тут
            # неоткуда взять.
            #
            # Слить два объекта можно только там, где доказательства и не
            # требуется: объект за проекцией сам заведён по этому пути и
            # носит тот же заголовок — значит перед нами та самая карточка,
            # которой наконец проставили `source_id`. Заголовок тут не улика
            # в пользу слияния (совпадение текста ею и не бывает, §4.3), а
            # улика против: разошёлся — карточка другая, и спорим.
            if проекция is None:
                спор = None
            elif row is not None:
                спор = "ключ" if проекция["id"] != row["id"] else None
            elif проекция["source_native_id"] != "vault:" + rel:
                спор = "ключ"
            elif проекция["title"] != (_строка(fm.get("title")) or None):
                спор = "заголовок"
            else:
                спор = None
            if спор:
                итог["спорных"] += 1
                print("ledger_import: %s стоит за объектом %s (ключ %s), а у "
                      "карточки source_id %s — разошёлся %s, не сливаем" %
                      (rel, проекция["id"], проекция["source_native_id"],
                       native, спор), file=sys.stderr)
                continue
            прежний = row["id"] if row else (
                проекция["id"] if проекция else None)
