...
def run(con, vault=None, dry_run=False):
    ...
    for подкаталог, вид, таблица, поля in ВИДЫ:
        ...
        for rel, fm, sha in карточки(vault, подкаталог):
            ...
            # собственным ключом объекта, который за ней стоит: без него
            # проекция говорит «здесь кто-то есть», но не говорит, наш ли.
            ...
            # Так выглядят сразу три случая, и различить их нечем: карточку
            # переименовали на путь другой, на месте удалённой завели новую,
            # `source_id` из карточки убрали. Правила, которое выбрало бы
            # между ними, нет: ТЗ §4.3 прямо требует доказательства, что это
            # одно событие, а его тут неоткуда взять.
            if проекция is not None and (
                    проекция["id"] != row["id"] if row is not None
                    else проекция["source_native_id"] != "vault:" + rel):
                итог["спорных"] += 1
                print("ledger_import: %s стоит за объектом %s (ключ %s), а "
                      "source_id карточки %s — не сливаем" %
                      (rel, проекция["id"], проекция["source_native_id"],
                       native), file=sys.stderr)
                continue
