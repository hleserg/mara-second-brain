...
def смонтирован(target, root):
    ...
    Оба бэкапных скрипта делают `mkdir -p` перед записью. Если внешний диск
    или сетевая шара отвалились, каталог просто создаётся заново на корневой
    ФС — запись проходит, скрипт рапортует успех, а третья копия по §12 лежит
    на том же физическом диске, что и первые две. Отказ при этом выглядит
    ровно как норма: `isdir` истинен, архив на месте, сверка молчит.

    Сравниваем `st_dev`, а не зовём `ismount`: интересует не «это точка
    монтирования», а «это другой диск» — на том же вопросе стоит и §12.
    Смотрим ближайшего существующего предка: на свежем носителе `mara/` ещё
    нет, а вот сама точка монтирования есть всегда — и смонтированная, и нет.
    ...
    while not os.path.exists(p) and p != os.path.dirname(p):
        p = os.path.dirname(p)
    return os.stat(p).st_dev != os.stat(root).st_dev
