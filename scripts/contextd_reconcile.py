...
def волт_пропал(vault):
    ...
    Промах монтирования ловится тем же: каталог есть, а каталогов карточек в
    нём нет. Пустой каталог карточек поломкой не считаем — так выглядит
    свежая установка, в которой проектор ещё не работал.
    """
    if not vault:
    ...
    if not os.path.isdir(vault):
        return [находка("волт-пропал", "error",
                        ...
                        % vault)]
    нет = [sub for sub in КАРТОЧКИ
           if not os.path.isdir(os.path.join(vault, sub))]
    if нет:
        return [находка("волт-пропал", "error",
                        "в волте нет каталогов карточек: %s — похоже, "
                        "смонтировано не то" % ", ".join(нет),
                        missing=нет)]
    return []
...
def _застава(имя, проверка, *a, **kw):
    ...
    try:
        ...
        return [находка("%s-не-проверена" % имя, "warn",
