...
def карантин(con, root):
    ...
    Два потолка, оба сознательные:

    * `blob_retention.py` карантин **не убирает** — он ходит по таблице
      `blobs`, а у карантинного файла строки там нет. Каталог растёт, пока
      владелец не разберёт его руками. Выбрасывать без разбора единственную
      серверную копию того, что телефон считал записью звонка, нельзя;
    * `core-backup.py` карантин **не копирует**: `МЕЛОЧЬ` — allowlist
      (`manifests`, `transcripts`, `extractions`), аудио идёт зеркалом из
      `calls/`. Потери тут нет: телефон локальную копию на `FAILED` не
      удаляет, а зашифрованное зеркало не место для содержимого, о котором мы
      знаем только то, что не понимаем его.
    """
    rows = con.execute(
        "select id, received from events where state='quarantined' "
        "order by received").fetchall()
    if not rows:
        return []
    старейшее = _возраст(rows[0]["received"])
    файлов = len(glob.glob(os.path.join(root, "quarantine", "*")))
    return [находка("карантин", "error",
                    "непринятых записей: %d (файлов в карантине: %d) — "
                    "проверить формат записи в ACR" % (len(rows), файлов),
                    count=len(rows), files=файлов,
                    oldest_seconds=int(старейшее or 0),
