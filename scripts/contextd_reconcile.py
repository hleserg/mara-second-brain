...
# Признак «это наш волт, а не чужой том». Берётся из того, что раскладывает
# установщик (`install/stage0-doctor.sh`: `raw` — строка 17, `entities` и
# `_system` — 19-20, `.git` — 47) и держит крон коммитов
# (`scripts/vault-git.sh:8` — замок в `$VAULT/.git/`), а не из того, что
# создаёт проектор. Каталоги карточек на эту роль не годятся: их заводит
# лениво `call_project.py` (`_atomic` → `os.makedirs`), и на живом волте
# doctor 2026-09-11 обоих нет вовсе — звонков ещё не было. Проверка на них
# сыпала бы `error` каждый час на здоровой системе, а на промахнувшемся
# монтировании гасла бы сама после первой же карточки. Один из четырёх —
# `raw` — в продакшене пересоздаст и `hermes-ingest.py:96` при первом
# зеркалировании; находку это не гасит, потому что она срабатывает на
# отсутствие **любого** из четырёх.
СКЕЛЕТ = (".git", "raw", "entities", "_system")
# А это каталоги карточек: их пишет проектор, и по ним сверяется индекс.
...
def волт_пропал(vault):
    """Волта нет — это поломка, а не «находок нет».

    `лаг_индекса` и `пакет_устарел` на отсутствующем каталоге возвращают
    пустой список, и размонтированный том из сверки неотличим от здоровой
    системы. Хуже: проектор создаёт каталог заново на системном диске и
    пишет карточки туда, а владелец открывает в Obsidian вчерашний слепок и
    не знает, что видит (#39).

    Промах монтирования ловится тем же: каталог есть, а скелета в нём нет.
    Свидетель — `СКЕЛЕТ`, то есть `.git` и каталоги установщика; почему не
    каталоги карточек, написано там же.
    """
    if not vault:
        return []                       # сверка без волта — законный режим
    if not os.path.isdir(vault):
        return [находка("волт-пропал", "error",
                        "волта нет: %s — том размонтирован или путь сменился"
                        % vault)]
    нет = [sub for sub in СКЕЛЕТ
           if not os.path.exists(os.path.join(vault, sub))]
    if нет:
        return [находка("волт-неполон", "error",
                        "в волте нет скелета: %s — похоже, смонтировано не то"
                        % ", ".join(нет), missing=нет)]
    return []


def лаг_индекса(vault, bm_db):
    """Расхождение волта и индекса Basic Memory в обе стороны.

    Не проиндексированные карточки — только счёт, чинить нечего: обход
    Basic Memory догонит сам. Пропавшие из волта — тот же счёт, но повод
    посмотреть, что унесло файлы.
    """
    if not vault:
        return []
    if not bm_db:
        return []                       # индекс не настроен — законный режим
    if not os.path.exists(bm_db):
        # Пропавший файл базы глушил **обе** стороны сравнения: ни
        # `лаг-индекса`, ни `карточки-пропали`. Тот же тихий отказ, от которого
        # написан `волт_пропал`, только на шаг ниже, и он же несущий: пока
        # сравнение шло в одну сторону, глушить было почти нечего, а теперь
        # обратная сторона — единственный свидетель пропавших карточек.
        # `BM_DB` (:82) имеет путь по умолчанию, поэтому «файла нет» — это
        # единственный достижимый в продакшене вид ненастроенности, и
        # различать его от «не настроено» приходится здесь.
        #
        # Действия названы оба, иначе находка была бы вечной на машине, где
        # Basic Memory и не ставили.
        return [находка("лаг-индекса", "warn",
                        "базы Basic Memory нет: %s — поднять "
                        "`basic-memory-mcp` или, если на этой машине индекса "
                        "быть не должно, задать `MARA_BM_DB=` пустым" % bm_db)]
    свои = set()
    for sub in КАРТОЧКИ:
        # обход рекурсивный, и это не запас на будущее. Пока сравнение шло в
        # одну сторону (`свои - видит`), неполнота перечисления прятала находку;
        # обратная сторона ниже из той же неполноты находку **создаёт**, и
        # карточка в подкаталоге висела бы в «пропали» вечно. Проектор сегодня
        # кладёт плоско (`call_project.py:106` и `:159` — имя собирается без
        # разделителей пути), но полагаться на это незачем.
        #
        # Потолок перечисления назван, раз обратная сторона теперь из него
        # находки **создаёт**: `glob` не видит точечных имён, а база видит, и
        # NFC/NFD база различает, а файловая система macOS — нет. И то и другое
        # дало бы незакрываемую находку, и то и другое сегодня недостижимо:
        # имена карточек собирает `call_project.slug()`, а он выдаёт только
        # `[a-z0-9-]` (проверено его же ассертом, `call_project.py:469`).
        for p in glob.glob(os.path.join(vault, sub, "**", "*.md"), recursive=True):
            свои.add(os.path.relpath(p, vault))
    # раньше здесь стоял `if not свои: return []`, и это был самый тихий отказ
    # из всех: волт, из которого карточки исчезли, не давал ни одной находки —
    # сравнивать было нечего. Сравнивать есть с чем: база помнит файлы.
    ...
    try:
        ...
        # Находка обязана просить о чём-то исполнимом, иначе она учит не читать
        # `warn`. Поэтому здесь не только текст ошибки: `no such table` значит,
        # что базу подменили или обновили схему, `database is locked` — что
        # `basic-memory-mcp` держит запись дольше десяти секунд таймаута. Оба
        # случая лечит одно и то же первое действие, и оно названо.
        return [находка("лаг-индекса", "warn",
                        "база Basic Memory не читается (%s) — посмотреть "
                        "`systemctl status basic-memory-mcp` и схему `entity`" % e)]
    finally:
        con.close()
    out = []
    нет = sorted(свои - видит)
    if нет:
        out.append(находка("лаг-индекса", "warn",
                           "не проиндексировано карточек: %d" % len(нет),
                           count=len(нет), sample=нет[:5]))
    # обратная сторона того же сравнения: база помнит карточку, которой в
    # волте больше нет. Молчание тут — то же молчание, от которого написан
    # `волт_пропал`, только переехавшее на уровень ниже: волт смонтирован,
    # скелет на месте, а карточки исчезли (промах rsync, откат git,
    # переименование каталога). Свидетель уже в руках — считать его стоит
    # одной строки. Пропавшие из индекса сами не чинятся: Basic Memory
    # выбрасывает запись при своём обходе, а обход бывает реже сверки,
    # поэтому уровень `warn`, а не `error`.
    # `k and` — против строки с `file_path is null`: до появления этой стороны
    # `None` был безвреден, теперь он роняет проверку и уносит с собой уже
    # посчитанную находку `лаг-индекса`. `.endswith` держит сравнение
    # симметричным: перечисляем только `*.md`, значит и спрашивать вправе
    # только про них — иначе любой другой индексируемый файл станет вечной
    # находкой.
    пропали = sorted(k for k in видит - свои
                     if k and k.endswith(".md")
                     and any(k.startswith(sub + "/") for sub in КАРТОЧКИ))
    if пропали:
        out.append(находка("карточки-пропали", "warn",
                           "база помнит карточек, которых нет в волте: %d"
                           % len(пропали),
                           count=len(пропали), sample=пропали[:5]))
    return out
...
def пакет_устарел(vault):
    ...
    try:
        ...
        # каждый час первый. Обоснование «иначе унесёт следующие проверки»
        # тут больше не работает: их держит `_застава` в `run()`. Застава
        # осталась ради диагноза — она называет сломанный импорт словами
        # («проверка пакета не запустилась: …»), а общая дала бы безымянное
        # `пакет-упала`. Тот же ход, что у `лаг_индекса` с `sqlite3.Error`:
        # деградировать до жалобы, а не падать.
...
def ретеншен_просрочен(con):
    ...
    try:
        ...
        # же причина остаться после `_застава` — диагноз по имени, а не цепочка
        # (цепочку держит `run()`). Опечатка в `MARA_RAW_DAYS`
...
def запись_не_долита(con, hours=24):
    ...
    По той же причине не попадают карантинные: файл дошёл целиком, но звуком
    не оказался, и строки в `blobs` у него не будет никогда. Их считает
    проверка `карантин` ниже — и считает как отказ, а не как ожидание."""
    rows = con.execute(
        ...
        "and e.state not in ('stale','quarantined') order by e.received").fetchall()
...
def карантин(con, root):
    """Содержимое дошло целиком, а звуком не оказалось (§6.2).

    Приём сверяет содержимое с закрытым списком видов (`contextd.нюх`) и
    непонятое кладёт в `<root>/quarantine/`, событие переводит в
    `quarantined`, телефону отвечает 415 — у него это терминально
    (`Core.kt:280`), локальную копию он не удаляет.

    Молчать тут нельзя: отказ приёма без находки — ровно то «молчание вместо
    поломки», от которого написан весь этот файл. Уровень `error`, потому что
    причина всегда снаружи кода и всегда требует человека: диктофон настроен
    на формат, которого нет в списке, либо в папку записей попало не то.

    **Считаем файлы, а не строки событий** — и вот почему это важнее, чем
    выглядит. Состояние `quarantined` из события уже не уходит: оно тупиковое
    по замыслу. Находка по строкам не погасла бы **никогда**, и мы получили бы
    ровно тот вечный `error`, который в этом же файле осуждён у
    `запись_не_долита` («вечное предупреждение заслоняет ровно то, ради чего
    проверка заведена») и который круг 1 ревью PR #90 забраковал у
    `волт_пропал`. Находка по файлам гаснет ровно тем действием, которого она
    просит: владелец разобрал каталог — находки нет. Счёт событий уходит в
    поле `events` справкой, а не условием.

    Возраст берётся из mtime файла, а не из `received` события: после
    частичной разборки каталога `received` соврал бы старейшим из всех
    когда-либо непринятых.

    Потолок: ни `blob_retention.py`, ни `core-backup.py` карантин не трогают —
    оба ходят по таблице `blobs`, а у карантинного файла строки там нет.
    Ретеншен его не подчистит, в зашифрованное зеркало он не уедет. Потери нет:
    телефон локальную копию на `FAILED` не удаляет, а зеркало не место для
    содержимого, о котором мы знаем только то, что не понимаем его.
    """
    файлы = sorted(glob.glob(os.path.join(root, "quarantine", "*")))
    if not файлы:
        return []
    события = con.execute(
        "select count(*) from events where state='quarantined'").fetchone()[0]
    возрасты = []
    for f in файлы:
        try:
            возрасты.append(time.time() - os.path.getmtime(f))
        except OSError:
            pass                       # разбирают прямо сейчас — не беда
    return [находка("карантин", "error",
                    "в карантине файлов: %d, старейшему %d ч — приём не понял "
                    "содержимое, проверить формат записи в ACR и разобрать "
                    "каталог" % (len(файлы), int(max(возрасты or [0]) // 3600)),
                    count=len(файлы), events=события,
                    oldest_seconds=int(max(возрасты or [0])),
                    sample=[os.path.basename(f) for f in файлы[:5]])]
...
def дайджест_догрузка(con, окно_ч=48):
    ...
    Окно — двое суток, и это не запас на всякий случай. Сводка владельцу
    уходит раз в день (`install/mara.cron`, `0 8 * * *`), а сверка обходит
    манифесты, волт и носители, так что до этой проверки доходит каждый раз
    в другую секунду. При окне ровно в сутки строка, легшая между вчерашним
    и сегодняшним моментом проверки, не попала бы в сводку никогда —
    подброшенная монета. Двое суток дают находке два утра.

    Дальше она обязана уйти сама: строки `stale` лежат в базе вечно, и
    вечная находка про них научила бы владельца не читать находки вовсе.
    Это сообщение о событии («приехала догрузка»), а не о поломке."""
    ...
    if not rows:
        return []
    # порог берём из источника, а не литералом: владелец вправе переопределить
    # его средой, и тогда «старше 24 ч» в сводке было бы враньём
    import call_digest as cd
    ids = [r["event_id"] for r in rows[:5]]
    return [находка("дайджест-догрузка", "warn",
                    ...
                    "MARA_DIGEST_MAX_AGE_H=999999 python3 "
                    "scripts/call_digest.py --event <id>, где id — один из: %s"
                    % (len(rows), cd.СВЕЖЕСТЬ_Ч, ", ".join(ids)),
                    count=len(rows), sample=ids)]
...
def _застава(имя, проверка, *a, **kw):
    """Упавшая проверка называет себя и не уносит остальные.

    Проверки звались подряд без застав, и исключение в третьей забирало
    все оставшиеся: цикл выходил ненулевым кодом, но что именно не
    проверено — в сводке не было (#39). Числа тут нет намеренно: проверок
    прибавляется, и «десять оставшихся» протухло бы на первом же новом
    `_застава(...)` в `run()` — а `run()` для этого и написан. Уровень `warn`, а не `error`:
    непроверенное — не то же, что сломанное, и крон не должен кричать об
    отвалившемся диске голосом «данные потеряны».

    Текст отказа обязателен: трассы в `reconcile.log` нет, и без него
    опечатка в env, пропавший файл и синтаксическая ошибка выглядят
    одинаково.

    Имя — `<имя>-упала`, а не `<имя>-не-проверена`: `пакет_устарел` и
    `ретеншен_просрочен` уже отдают `пакет-не-проверен` и
    `ретеншен-не-проверен` (ленивый импорт соседа не поднялся), и любой grep
    по короткому имени поймал бы длинное подстрокой. Смыслы разные: там
    проверка отработала и сказала «не знаю», здесь не запустилась вовсе.
    """
    try:
        return проверка(*a, **kw)
    except Exception as e:
        return [находка("%s-упала" % имя, "warn",
                        "%s: проверка не запустилась: %s: %s"
                        % (имя, type(e).__name__, e))]


def run(con, root=None, vault=VAULT, bm_db=BM_DB, targets=None):
    root = root or mi.ROOT
    out = []
    # Волт первым: если его нет, половина находок ниже пустая не потому, что
    # всё хорошо.
    out += _застава("волт", волт_пропал, vault)
    out += _застава("манифесты", манифест_без_блоба, con, root)
    out += _застава("блобы", блоб_без_манифеста, con, root)
    out += _застава("расшифровка", запись_без_расшифровки, con, root)
    out += _застава("извлечение", транскрипт_без_извлечения, con, root)
    out += _застава("индекс", лаг_индекса, vault, bm_db)
    out += _застава("пакет", пакет_устарел, vault)
    out += _застава("ретеншен", ретеншен_просрочен, con)
    out += _застава("сердцебиение", сердцебиение, root)
    out += _застава("источники", источник_замолчал, con)
