...
# установщик (`install/stage0-doctor.sh:17-18`) и держит крон коммитов
...
# монтировании гасла бы сама после первой же карточки.
...
def лаг_индекса(vault, bm_db):
    ...
    пропали = sorted(k for k in видит - свои
                     if any(k.startswith(sub + "/") for sub in КАРТОЧКИ))
    if пропали:
