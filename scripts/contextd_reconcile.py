...
# суток молчания это не стоит. 0.2 суток запаса — на задержку самой пересборки.
# Прогон сверки в 7 минут каждого часа: после пропущенной ночи первая жалоба
# в 10:07, во владельческую сводку в 8:00 находка попадает со второй ночи.
ПАКЕТ_СУТКИ = 1.2
...
def пакет_устарел(vault):
    ...
    `now.md` писателей два: наш `build_now` и Basic Memory, дописывающий свой
    фронтматтер при синке волта (ADR-0008, решения 6 и 7). Его синк подвинул
    бы `mtime` в ночь, когда пересборка не состоялась, и проверка молчала бы
    ровно там, где заведена. Манифест пишет только `build_now` и только под
    тем же замком — один писатель, честный признак живости.
