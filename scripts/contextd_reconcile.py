...
def запись_не_долита(con, hours=24):
    ...
    очередь загрузки на телефоне против серверной базы
    (§17 `TZ-ambient-memory.md`). Свежий
...
def дайджест_не_доставлен(con):
    ...
    нём не узнал (§17 `TZ-ambient-memory.md`). Оба недоставленных
    состояния — настроечные, и оба
    чинятся одним файлом `/etc/mara/contextd.env`: `no-transport` — нет токена
    или адресата вовсе, `not-private` — адресат есть, но это не личный чат
    владельца, и застава §8.3 `TZ.md` отправку не пустила (#61).
