...
def бэкап_ядра(targets=None, days=БЭКАП_СУТКИ, root=None, ошибки=None):
    ...
    out = []
    if ошибки:
        # `return`, а не вторая находка: неразобранный список носителей и
        # пустой список — это одна беда и её следствие, а две строки об
        # одном в дайджесте уже шум. Побочно: без `return` находку о
        # пустом списке нельзя отличить от находки о разборе — обе под
        # одним именем, и тест на разбор проходил бы при выключенном разборе.
        return out + [находка("бэкап-ядра-конфиг", "error",
