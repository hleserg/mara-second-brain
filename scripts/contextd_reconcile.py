...
def бэкап_ядра(targets=None, days=БЭКАП_СУТКИ, root=None):
    ...
    if not с_копией:
        ...
        # Без «потому что»: причина у каждого носителя своя и уже названа
        # его собственной строкой, а короткий хвост вроде «не смонтирован или
        # пуст» лжёт на третьем пути сюда — носитель смонтирован и не пуст,
        # на нём лежит один сайдкар.
        out.append(находка("бэкап-ядра-копий-ноль", "warn",
                           "внешних копий ядра сейчас ноль: ни один из "
                           "носителей (%s) не отдаёт архив ядра"
                           % " ".join(targets)))
    return out
