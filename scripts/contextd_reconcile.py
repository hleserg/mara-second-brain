...
try:
    НОСИТЕЛИ, КОНФИГ_БИТ = mi.носители(os.environ.get(
        "MARA_CORE_TARGETS", "/mnt/backup/mara /mnt/win-backups/mara")), None
except ValueError as e:
    # Опечатка в одной переменной не имеет права уносить всю сверку: без неё
    # молчат и DLQ, и сердцебиения, и ретеншен. Докладываем находкой.
    НОСИТЕЛИ, КОНФИГ_БИТ = [], str(e)
ОТМЕТКА = os.path.join(mi.STATE, "core-targets.json")
БЭКАП_СУТКИ = float(os.environ.get("MARA_CORE_BACKUP_MAX_DAYS", 2))
# Порог отвала отдельный и в сутках больше: бэкап пишет раз в ночь,
# и два дня — это всего одна пропущенная запись. Машину, выключенную
# на выходные, такой порог объявлял бы отказом каждое воскресенье.
ОТВАЛ_СУТКИ = float(os.environ.get("MARA_CORE_TARGET_MAX_DAYS", 3))
...
def бэкап_ядра(targets=None, days=БЭКАП_СУТКИ, root=None, отметка=None,
               отвал=ОТВАЛ_СУТКИ, конфиг_бит=None):
    ...
    принимавшая архив дольше `отвал`, — отказ. Порог отдельный от `days`:
    запись идёт раз в ночь, и мерить простой носителя тем же числом, что и
    возраст архива, значит объявлять отказом одну пропущенную ночь.
    ...
    if конфиг_бит is None:
        конфиг_бит = КОНФИГ_БИТ
    if конфиг_бит:
        return [находка("бэкап-ядра-конфиг", "error",
                        "MARA_CORE_TARGETS не разобрать: %s" % конфиг_бит)]
    ...
    if targets and not живых and not out:
        # Только когда по каждому носителю в отдельности сказать нечего:
        # иначе к «носитель X отвалился» пристёгивалась бы вторая строка
        # ровно о том же. Ради этой ветки она и написана — молчание при
        # свежих отметках на всех носителях сразу.
        out.append(находка("бэкап-ядра-носители", "warn",
