...
def ретеншен_просрочен(con):
    ...
    try:
        ...
        # же цена отказа. `blob_retention` на модульном уровне приводит
        # `MARA_RAW_DAYS` к int, так что опечатка в задокументированной
        # переменной роняла отсюда всю сверку — шесть проверок ниже вместе
        # со сводкой в 8:00 (у соседа выше их семь: число тут своё).
        # Саму уборку это не чинит: у неё свой крон, свой процесс и свой
        # отказ, и он остаётся в #39.
