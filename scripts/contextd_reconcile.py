#!/usr/bin/env python3
"""Сверка состояния приёма (спека §9, §17 `TZ-ambient-memory.md`).

Крон раз в час. Инварианты приёма. То, что чинится однозначно, сверка чинит
сама: ставит пропущенную работу извлечения, снимает с ретраев работу, у
которой пропал исходник. То, где нужен человек, она только докладывает —
файлы не удаляет никогда, даже осиротевшие: единственная копия личного
разговора стирается по ретеншену или по прямой команде, а не по догадке.

`--telegram` — раз в день, только о реальных проблемах, и ничего, если их
нет: DLQ не должен превращаться в админку (§17 `TZ-ambient-memory.md`).
...
# следующим утром — вторыми сутками молчания там, где ADR-0008
# (решение 5) обещает первые: «пакет старше суток при живом кроне —
...
def запись_без_расшифровки(con, root):
    ...
    Дыра §5.2 `TZ-master.md` в чистом виде. `finish_stored` переводит
    состояние, потом пишет манифест, потом ставит работу — тремя отдельными
    коммитами, транзакции вокруг них нет (ADR-0005). Смерть демона между
    первым и третьим оставляет запись, которую никто не расшифрует, и сам по
    себе повтор не спасает: перевод уже сделан, и `update ... where
    state='new'` больше не сработает.
...
def пакет_устарел(vault):
    ...
    try:
        ...
        # семь следующих — ретеншен, сердцебиение, DLQ, бэкап §10 `TZ.md` — и
        # сводка в 8:00 не ушла бы вовсе. Тот же ход, что у `лаг_индекса` с
...
def запись_не_долита(con, hours=24):
    ...
    очередь загрузки на телефоне против серверной базы (§17
    `TZ-ambient-memory.md`). Свежий звонок ещё может долиться с плохой сети,
...
def дайджест_не_доставлен(con):
    """Дайджест собран, но до владельца не дошёл: звонок разобран, а человек о
    нём не узнал (§17 `TZ-ambient-memory.md`). Оба недоставленных состояния
    настроечные, и оба чинятся одним файлом `/etc/mara/contextd.env`:
    `no-transport` — нет токена или адресата вовсе, `not-private` — адресат
    есть, но это не личный чат владельца, и застава §8.3 `TZ.md` отправку не
    пустила (#61).
    `failed` уводит работу в ретрай, и если та встанет насовсем — о ней
    скажет dlq(), а дублировать одну беду двумя находками ни к чему.

    Находка одна на оба состояния: ремедия у них общая, а какого рода дыра —
    видно по счётчикам в тексте. Своей доставкой эта находка не спасается:
    сводка уходит тем же `deliver` и при кривом адресате отвергается той же
    заставой. Смысл её в том, что она **не протухает**: строки в `digests`
    лежат до починки, и первая же суточная сводка после неё их назовёт."""
    rows = con.execute("select event_id, state from digests "
                       "where state in ('no-transport','not-private') "
                       "order by sent_at").fetchall()
    if not rows:
        return []
    без = sum(1 for r in rows if r["state"] == "no-transport")
    чужой = len(rows) - без
    чем = []
    if без:
        чем.append("нет токена или адресата: %d" % без)
    if чужой:
        чем.append("адресат не личный чат владельца: %d" % чужой)
...
def бэкап_ядра(targets=None, days=БЭКАП_СУТКИ, root=None):
    ...
    по §10 `TZ.md` не было всю неделю, и сверка молчала — копия-то есть. Теперь
    молчание означает, что по каждому носителю отдельно всё сошлось: либо на
    нём свежий архив, либо он писался недавно и лишь отключён. Числа копий
    молчание не обещает — отключённая на вечер шара молчит законно, а копии
    на ней нет; обещает оно только то, что копий не ноль.
    ...
    как раньше отвечал первый же смонтированный носитель. Считаются копии, а не
    ...
    if ошибки:
        out.append(находка("бэкап-ядра-конфиг", "error",
                           ...
                           % "; ".join(ошибки)))
    if not targets:
        # Пустой список — не «носителей нет», а «третьей копии по §10 `TZ.md`
        # быть неоткуда». Молчать об этом нельзя тем более, что теперь
        # переменные живут в crontab, где их легко затереть пустым значением.
        if not ошибки:
            # Иначе вторая строка про ту же беду: пустой список — следствие
            # неразобранного списка, и обе находки под одним именем.
            out.append(находка("бэкап-ядра-конфиг", "error",
                               "список носителей пуст — третью копию по "
                               "§10 TZ.md класть некуда"))
        ...
        # уходя по ней, сверка выключала бы весь §10 `TZ.md` из-за опечатки в
        # цифре.
        return out
        ...
        if not os.path.isdir(t) or not mi.смонтирован(t, root):
            ...
            if простой > days:
                out.append(находка("бэкап-ядра-отвалился", "error",
                                   ...
                                   "копии по §10 TZ.md нет, смотреть "
                                   "монтирование"
                                   % (t, простой), target=t,
    ...
    if not с_копией:
        ...
        # Это единственная строка, называющая состояние по §10 `TZ.md` целиком.
        # Хвост нужен: есть вход, где своей строки нет ни у кого — все
        # носители отвалились, но писали недавно, и каждый молчит как
        # «шара, отключённая на вечер». Тогда эта строка приходит в дайджест
        # одна, и без хвоста владельцу не с чего начать. «Без архива», а не
        # «пуст»: носитель с одним сайдкаром не пуст, но архива не отдаёт.
        out.append(находка("бэкап-ядра-копий-ноль", "warn",
                           "внешних копий ядра сейчас ноль: ни один из "
                           "носителей (%s) не отдаёт архив ядра — не "
                           "смонтирован или без архива" % " ".join(targets)))
    return out
