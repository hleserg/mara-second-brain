...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    if drill:
        ...
        #    разница лишь в том, на какой миллисекунде его заметили. Промах
        #    учения при этом не случаен, а гарантирован, и по двум разным
        #    причинам. На железе — потому что после отвала точка монтирования
        #    пуста: нет НИ ОДНОГО файла зеркала, и порядок обхода не важен
        #    вовсе. При флапе, когда файлы вернулись, — потому что зеркало
        #    идёт по `blobs` без `order by`, а учение берёт `order by created
        #    desc limit ПРОБА`, то есть как раз хвост за точкой обрыва.
        #    Второй довод слабее первого: он держится на том, что `created`
        #    растёт вместе с rowid, а бэкфилл задним числом это сломает.
        #
        # Вторая и третья на боевом железе открываются только при флапе:
        # обычно вместе с монтированием уезжает и архив, и учение уходит к
        # соседу выше по коду. Но правило «не сверять то, чего не зеркалили»
        # одно на все три, и держать в коде три незачем.
        пропуск = зв.get(цель, {})
        без_зеркала = None
        if not аудио:
...
def самопроверка():
    ...
    try:
        ...
        # вызовов держится этой строкой и только ею.
        assert "архив развернуло" not in перехват10.getvalue(), \
