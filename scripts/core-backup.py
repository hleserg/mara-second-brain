...
def самопроверка():
    ...
    try:
        ...
        # Другой месяц, а не текущий: только так наблюдаемо, что проверка
        # стоит ПЕРЕД `makedirs`, а не после него. Все остальные блобы
        # игрушки лежат в одном каталоге, он уже создан под первым файлом, и
        # `makedirs(..., exist_ok=True)` для них — пустая операция. Мутант,
        # опустивший проверку под `makedirs`, на таком наборе неотличим, а в
        # бою оставил бы пустое дерево `calls/ГГГГ/ММ` на корневой ФС.
        ...
        # Каталог месяца под второй файл не создан: проверка стоит перед
        # `makedirs`, а не после. Иначе на носителе, которого нет, оставался
        # бы пустой `calls/2025/01`.
        assert not os.path.exists(os.path.dirname(зерк_s5)), зерк_s5
        шум = перехват.getvalue()
