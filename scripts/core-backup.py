...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    if битые:
        ...
        print(json.dumps(итог, ensure_ascii=False, indent=1, sort_keys=True),
              file=sys.stderr)
        raise RuntimeError("зеркало аудио: не записано %s"
                           % "; ".join("%s — %d" % (t, n)
...
def самопроверка():
    ...
    try:
        ...
        # Наблюдаема только сама просьба, её и считаем. Заодно третья ночь с
        # целым `шифр` обязана положить файл, который две прошлые не клали.
        целый_сброс = мод.сбросить_кеш
        просили = []
        мод.сбросить_кеш = lambda п: (просили.append(п), целый_сброс(п))[1]
        try:
            вернули = зеркало_аудио(os.path.join(root, "contextd.db"), root,
                                    второй, пароль)
        finally:
            мод.сбросить_кеш = целый_сброс
        assert вернули == {"новых": 1, "байт": вернули["байт"], "битых": 0,
                           "убрано_вычищенных": 0, "без_файла": 0}, вернули
        assert просили == [зерк2 + ".tmp"], просили
        ...
        # найдёт. Файл на диске оставляем, как и у первой записи.
        con = mi.connect(root)
