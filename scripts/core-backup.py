...
def самопроверка():
    ...
    try:
        ...
        # Ничего мимо носителя: каталог зеркала на корневой ФС не появился.
        # Ради этой строки всё и заведено — на отвалившемся носителе беда не
        # в том, что файла нет, а в том, что он есть не там.
        assert not glob.glob(os.path.join(tmp, "calls")), tmp
        con = mi.connect(root)
        con.execute("update blobs set purged_at=? where sha256=?",
                    (mi.now_iso(), s5))
        con.close()
