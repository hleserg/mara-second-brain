...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    if drill:
        ...
        # Учение сверяет то, что зеркалили (#63). Две двери ведут в одно и
        # то же: `--no-audio` (зеркала не делали вовсе) и носитель, что
        # отвалился между архивом и зеркалом, — а учение идёт по первому
        # записанному, то есть именно по нему. Прежде обе кончались
        # `RuntimeError: аудио … нет в зеркале`, то есть падением ночи
        # из-за работы, которой не просили.
        # Причина называется вслух в двух местах сразу: в итоге и в `stderr`.
        # Молчание тут опаснее падения — пропиши кто-нибудь `--no-audio` в
        # крон навсегда, и учение перестанет проверять аудио, а зелёное
        # выглядит одинаково.
        без_зеркала = None
        if not аудио:
            без_зеркала = "зеркала не делали (--no-audio)"
        elif "пропущен" in зв.get(записано[0], {}):
            без_зеркала = "носитель не смонтирован при зеркалировании"
        if без_зеркала:
            print("core-backup: учение развернёт архив, но аудио не сверялось "
                  "— %s" % без_зеркала, file=sys.stderr)
...
def самопроверка():
    ...
    try:
        ...
        # Вторая дверь в ловушку #63, и `--no-audio` про неё ничего не знает:
        # отвалился не любой носитель, а ПЕРВЫЙ — тот самый, по которому идёт
        # учение. Зеркала на нём нет по своей же защите (`пропущен`), а
        # учение прежде требовало оттуда аудио и роняло ночь целиком.
        ...
        # А напарник до зеркалирования дошёл: пропуск головного не отменяет
        # работы на здоровых носителях. Счёт новых тут не проверяем — к
        # последнему сценарию блобы уже вычищены прежними ночами, зеркалить
        # нечего, и ноль здесь честный.
        assert "пропущен" not in r7["аудио"][напарник], r7
