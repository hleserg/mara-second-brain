...
def сказать_про_пропуск(причина):
    ...
    print("core-backup: учение развернёт архив, но аудио не сверялось — %s"
...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    if drill:
        ...
        #    разница лишь в том, на какой миллисекунде его заметили, — а
        #    промах при этом не случаен, а гарантирован: зеркало идёт по
        #    `blobs` без `order by`, то есть свежайшие последними, а учение
        #    берёт `order by created desc limit ПРОБА` — ровно тот хвост,
        #    который стоит за точкой обрыва.
        #
...
def самопроверка():
    ...
    try:
        ...
        # различения которого #63 и заводили.
        assert "аудио_сверено" in r8["проверка"], r8
