...
def зеркало_аудио(db, root, target, пароль):
    ...
    try:
        for r in con.execute("select sha256, path, purged_at from blobs"):
            ...
            if os.path.exists(рядом):
                continue
            # Носитель мог отвалиться уже посреди зеркала: проверка в
            # `прогон` стоит один раз, до цикла, а цикл идёт по всем блобам и
            # тянется секундами в штатную ночь и минутами при бэклоге. После
            # отвала точка монтирования пуста, и `os.path.exists(рядом)`
            # ложно не только для новых файлов, но и для копий, лежавших в
            # зеркале месяцами: без этой строки на корневую ФС уехал бы весь
            # хвост набора от точки отвала, а инкрементальность, ради которой
            # имя файла сделано хешем, защищать перестала бы.
            if not mi.смонтирован(target, root):
                print("core-backup: %s отвалился посреди зеркала, дальше не "
                      "пишу" % target, file=sys.stderr)
                оборвано = "носитель не смонтирован"
                break
            try:
        ...
        con.close()
    итог = {"новых": новых, "байт": байт, "убрано_вычищенных": убрано,
            "без_файла": без_файла, "битых": битых}
    if оборвано:
        # Ключ появляется только при обрыве: у штатной ночи форма ответа
        # прежняя, и застава на неё сравнивает словарь целиком.
        итог["оборвано"] = оборвано
    return итог
...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    try:
        ...
        for t in targets:
            ...
            try:
                ...
                # появляется ни на секунду, и §10 `TZ.md` не зачтёт её за
                # третью.
    ...
    if аудио:
        for t in записано:
            # Носитель мог отвалиться уже после того, как на него лёг архив:
            # `зеркало_аудио` делает свой `makedirs` и про монтирование не
            # спрашивает, так что зеркало легло бы на корневую ФС, а
            # `проверить_аудио` прочитало бы его оттуда же и зазеленело. Дыра
            # узкая — отвалиться надо внутри одного прогона, — но зелёная
            # сверка на потерянном диске хуже красной на живом.
            if not mi.смонтирован(t, root):
                print("core-backup: %s отвалился между архивом и зеркалом — "
                      "аудио не зеркалю" % t, file=sys.stderr)
                зв[t] = {"пропущен": "носитель не смонтирован"}
                continue
...
def самопроверка():
    ...
    try:
        ...
        # Расшифрованному аудио незачем оказываться на диске — ни в рабочем
        # каталоге, ни на носителе рядом с зеркалом (ТЗ §18). Держится это
        # тем, что оба чтения аудио идут трубой, а наблюдаемо только в argv:
        # `-o` рядом с `-d` и есть выкладывание плейнтекста файлом. Оракул
        # узкий и заявлен таким: он видит `-o`, а не всякий способ сохранить
        # поток. Вызовы копим с первого прогона: путь записи исполняется
        # только тут, второй прогон новых файлов не кладёт вовсе.
        мод = sys.modules[__name__]
        целый_вызов = мод.гпг_вызов
        вызовы = []
        мод.гпг_вызов = lambda a, п: (вызовы.append(list(a)),
                                      целый_вызов(a, п))[1]
        try:
            r = прогон(**общее)
        finally:
            мод.гпг_вызов = целый_вызов
        писатель.close()
        ...
        аудио_вызовы = [a for a in вызовы
                        if "-d" in a and any("calls" in x for x in a)]
        # Обе половины обещания обязаны быть в выборке, иначе застава держит
        # ту, которую этот PR не добавлял: `-d` по временному имени — сверка
        # при записи, `-d` по боевому — учение. Новый файл кладёт только
        # первый прогон, разворачивает — каждый.
        assert [a for a in аудио_вызовы if a[-1].endswith(".tmp")], вызовы
        assert [a for a in аудио_вызовы if a[-1].endswith(".gpg")], вызовы
        for a in аудио_вызовы:
            assert "-o" not in a, a
        ...
        целый_шифр = мод.шифр
        # Здоровый носитель ПОСЛЕ сбойного. Без него обрыв обхода по
        # носителям неотличим от штатной работы: сбойный `второй` в списке
        # последний, и `break` после него ничего не меняет. Тот же дефект,
        # что круг 1 нашёл в цикле по файлам, только этажом выше.
        тыл = os.path.join(tmp, "target6")
        os.makedirs(тыл, mode=0o700)
        цел6 = os.path.join(тыл, os.path.relpath(p2, root) + ".gpg")
        трое = dict(общее, targets=[target, второй, тыл])
        ...
        for кривой, слово in ((не_gpg, "не gpg"), (не_тот_файл, "не то аудио")):
            # Обманки на все три носителя, а не только на здоровый: ради
            # ротации именно на СБОЙНОМ носителе падение и отодвинуто в
            # конец, и проверять её на нём же.
            for место in (target, второй, тыл):
                for дата in ("1999-01-01", "1999-01-02", "1999-01-03"):
                    стар = os.path.join(место, "core-%s.tar.gz.gpg" % дата)
                    open(стар, "w").close()
                    os.chmod(стар, 0o600)
            # Зеркало тыла убираем перед каждой порчей: лежащий файл
            # `зеркало_аудио` не перечитывает, и застава ниже прошла бы на
            # копии от прошлой итерации, не проверив ничего.
            if os.path.exists(цел6):
                os.unlink(цел6)
            мод.шифр = кривой
            ...
            # учения из-за одного файла не теряются. Ключи перечислены, а не
            # взяты парой: заглушка вида `{"битых": 1, "проверка": {}}` — не
            # итог ночи, а его видимость.
            for ключ in ('"битых"', '"проверка"', '"носители"', '"архив"',
                         '"счётчики"', '"секунд"'):
                assert ключ in шум, (слово, ключ, шум[-400:])
            # Носитель за сбойным обязан получить свой файл: отказ одного
            # носителя не имеет права унести соседний.
            assert os.path.exists(цел6), (слово, цел6)
            ...
            for место in (target, второй, тыл):
                assert not glob.glob(os.path.join(место,
                                                  "core-1999-01-0[12]*")), \
                    (слово, место)
            # Файла в зеркале нет — значит следующая ночь попробует снова.
            os.unlink(цел2)

        # #54: между записью архива и зеркалом носитель может отвалиться.
        # `зеркало_аудио` делает свой `makedirs` и про монтирование не
        # спрашивает — зеркало ляжет на корневую ФС, `проверить_аудио`
        # прочитает его оттуда же, и сверка зазеленеет на потерянном диске.
        # Отваливаем `второй` не по счёту вызовов, а по факту: носитель
        # считается живым, пока на нём нет свежего архива. Так застава
        # описывает саму беду («отвалился между архивом и зеркалом»), а не
        # порядок вызовов, и переживает перестановку строк.
        # Архивы прошлых прогонов с носителя убираем: «на нём уже есть
        # сегодняшний архив» и должно означать «его положил этот прогон».
        # Маска `core-2*`, а не `core-*`: обманки ротации выше названы
        # `core-1999-*` и попасть под неё не должны.
        for ф in glob.glob(os.path.join(второй, "core-2*")):
            os.unlink(ф)
        # Зеркало тыла убираем: последняя итерация порчи его положила, и без
        # этого застава «здоровый носитель ПОСЛЕ отвалившегося своё получил»
        # прошла бы на файле от прошлого прогона, не проверив ничего.
        if os.path.exists(цел6):
            os.unlink(цел6)
        целый_смонтирован = mi.смонтирован

        def отвалился_после_архива(t, корень):
            if t == второй and glob.glob(os.path.join(t, "core-2*.tar.gz.gpg")):
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = отвалился_после_архива
        перехват = io.StringIO()
        try:
            with contextlib.redirect_stderr(перехват):
                # Три носителя, а не два: отвалившийся обязан стоять НЕ
                # последним, иначе обрыв всего обхода (`break` вместо
                # `continue`) неотличим от штатной работы и мутант выживает.
                # Та же находка, что круг 1 сделал по циклу файлов в #45.
                r4 = прогон(**трое)
        finally:
            mi.смонтирован = целый_смонтирован
        # Архив на носитель лёг — отваливаться он начал позже, и это часть
        # сценария, а не оговорка: носитель обязан остаться в `носители`.
        assert r4["носители"] == [target, второй, тыл], r4
        assert os.path.exists(os.path.join(второй, r4["архив"])), r4
        # Главное: зеркала на отвалившемся носителе не появилось.
        assert not os.path.exists(зерк2), зерк2
        assert r4["аудио"][второй] == {"пропущен": "носитель не смонтирован"}, r4
        # А здоровый носитель своё зеркало получил: пропускаем отвалившийся,
        # а не бросаем зеркало целиком.
        assert r4["аудио"][target]["новых"] == 1, r4
        assert os.path.exists(цел2), цел2
        # И носитель ЗА отвалившимся — тот самый тыл: пропуск одного не имеет
        # права унести зеркало остальных.
        assert r4["аудио"][тыл]["новых"] == 1, r4
        assert os.path.exists(цел6), цел6
        шум = перехват.getvalue()
        # Носитель обязан быть назван поимённо и сказано, что зеркала не
        # было: в итоге стоит только слово «пропущен», и без строки в
        # `stderr` ночная тишина неотличима от штатной работы.
        assert второй in шум and "не зеркалю" in шум, шум[-400:]
        os.unlink(цел2)
        ...
        # Файл проверяем до словаря: иначе его отсутствие вылезло бы
        # `FileNotFoundError` из построения ожидания, а не заставой.
        assert os.path.exists(зерк2), зерк2
        # `байт` берётся с самого файла, а не из ответа: `вернули["байт"]`
        # на месте ожидания сравнивал поле само с собой, и счётчик, который
        # больше нигде не проверяется, не держала ни одна строка.
        assert вернули == {"новых": 1, "байт": os.path.getsize(зерк2),
                           "битых": 0, "убрано_вычищенных": 0,
                           "без_файла": 0}, вернули
        assert просили == [("сброс", зерк2 + ".tmp"),
                           ("хеш", зерк2 + ".tmp")], просили

        # Ротация стоит последней не для порядка: свежий архив, не прошедший
        # учение, не имеет права вытеснить старый, который разворачивался.
        # Свойство пришло с #44 и до сих пор не держалось ничем — мутант,
        # поднявший блок ротации выше учения, проходил и гейт, и самопроверку.
        for дата in ("1999-01-01", "1999-01-02", "1999-01-03"):
            стар = os.path.join(target, "core-%s.tar.gz.gpg" % дата)
            open(стар, "w").close()
            os.chmod(стар, 0o600)
        целая_проверка = мод.проверка

        def учение_упало(*a, **kw):
            raise RuntimeError("учение упало")

        мод.проверка = учение_упало
        try:
            прогон(**общее)
            raise AssertionError("упавшее учение прошло молча")
        except RuntimeError as e:
            assert "учение упало" in str(e), str(e)
        finally:
            мод.проверка = целая_проверка
        # Обманки обязаны остаться на месте: ротации не было вовсе.
        assert len(glob.glob(os.path.join(target, "core-1999-*"))) == 3, \
            glob.glob(os.path.join(target, "core-1999-*"))
        for ф in glob.glob(os.path.join(target, "core-1999-*")):
            os.unlink(ф)
        ...
        # Другой месяц, а не текущий: на этом блобе рвётся цикл в заставе #57
        # ниже, и только чужой каталог месяца делает наблюдаемым, что сторож
        # стоит ПЕРЕД `makedirs`. Здешним проверкам месяц безразличен.
        p3 = mi.blob_path(root, s3, "wav",
                          when=datetime(2025, 1, 15, tzinfo=mi.TZ))
        ...
        # Та же поимённая строка, что и у неудавшейся записи: счёт в итоге
        # один на четыре разных отказа, и без неё не понять, который случился.
        шум = перехват.getvalue()
        assert "не убрано" in шум and s4[:12] in шум, шум[-400:]
        os.rmdir(зерк4)
        ...
        # #57: носитель может отвалиться и посреди зеркала. Проверка в
        # `прогон` стоит один раз, до цикла, а цикл идёт по всем блобам.
        # После отвала точка монтирования пуста, и `os.path.exists(рядом)`
        # ложно не только для новых файлов, но и для копий, лежавших
        # месяцами: без проверки внутри цикла на корневую ФС уехал бы весь
        # хвост набора от точки отвала.
        шестой = os.path.join(tmp, "target6-mid")
        os.makedirs(шестой, mode=0o700)
        тело5 = b"audio-number-five"
        s5 = hashlib.sha256(тело5).hexdigest()
        p5 = mi.blob_path(root, s5, "wav")
        os.makedirs(os.path.dirname(p5), exist_ok=True)
        open(p5, "wb").write(тело5)
        con = mi.connect(root)
        con.execute("insert into blobs(sha256,path,bytes,mime,created)"
                    " values(?,?,?,?,?)",
                    (s5, p5, len(тело5), "audio", mi.now_iso()))
        con.close()
        зерк_s2 = os.path.join(шестой, os.path.relpath(p2, root) + ".gpg")
        зерк_s3 = os.path.join(шестой, os.path.relpath(p3, root) + ".gpg")
        зерк_s5 = os.path.join(шестой, os.path.relpath(p5, root) + ".gpg")

        # Отваливаем по факту, а не по счёту вызовов: носитель жив, пока на
        # него не лёг первый файл зеркала. Так застава описывает беду
        # («отвалился посреди зеркала»), а не порядок строк.
        def отвалился_посреди(t, корень):
            if t == шестой and glob.glob(os.path.join(t, "calls", "*", "*",
                                                      "*.gpg")):
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = отвалился_посреди
        перехват = io.StringIO()
        try:
            with contextlib.redirect_stderr(перехват):
                вернули = зеркало_аудио(os.path.join(root, "contextd.db"),
                                        root, шестой, пароль)
        finally:
            mi.смонтирован = целый_смонтирован
        # Первый файл лёг на живой ещё носитель — это не оговорка, а часть
        # сценария: обрыв обязан начинаться с той записи, на которой носитель
        # ушёл, а не отменять уже сделанное.
        assert os.path.exists(зерк_s2), зерк_s2
        assert вернули["новых"] == 1, вернули
        # А дальше — ни одного файла. Между `s2` и `s5` в обходе стоит `s3` с
        # чужим хешем: до правки он дал бы `битых: 1` на носителе, которого
        # уже нет, то есть жалобу не про ту беду.
        assert not os.path.exists(зерк_s5), зерк_s5
        assert вернули["битых"] == 0, вернули
        assert вернули["оборвано"] == "носитель не смонтирован", вернули
        # Каталог месяца под блоб, НА КОТОРОМ оборвались, не создан: сторож
        # стоит перед `makedirs`, а не после. Мерить это можно только здесь:
        # до правки застава стояла на `s5`, до которого цикл не доходит ни в
        # правильном коде, ни у мутанта, — и мутанта, опустившего сторож под
        # `makedirs`, не ловила вовсе. В бою он оставлял бы пустое дерево
        # `calls/ГГГГ/ММ` на корневой ФС под точкой монтирования.
        assert not os.path.exists(os.path.dirname(зерк_s3)), зерк_s3
        шум = перехват.getvalue()
        assert шестой in шум and "посреди зеркала" in шум, шум[-400:]
        # Ровно одна жалоба, а не по одной на каждый оставшийся блоб. Это и
        # есть разница между `break` и `continue` в обрыве: с `continue`
        # отвал на третьем файле из полусотни даёт 48 строк в
        # `core-backup.log` про одну и ту же беду.
        assert шум.count("посреди зеркала") == 1, шум.count("посреди зеркала")
        con = mi.connect(root)
        con.execute("update blobs set purged_at=? where sha256=?",
                    (mi.now_iso(), s5))
        con.close()
