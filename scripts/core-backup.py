...
БЕЗ_ЗЕРКАЛА_ФЛАГ = "зеркала не делали (--no-audio)"
...
def сказать_про_пропуск(причина):
    """Назвать причину вслух: в отчёте её видит машина, здесь — человек.

    Зовут оба пути, где аудио не сверяется, — ночь и `--drill-only`. Молчание
    тут опаснее падения: пропиши кто-нибудь `--no-audio` в крон навсегда, и
    зелёное без этой строки выглядело бы ровно как штатная ночь.

    Зовут ПОСЛЕ учения, а не до. До — строка обещала бы, что архив
    развернётся, и через мгновение оказывалась бы неправдой: учение по
    носителю, с которого архив уехал, падает «нет архивов ядра» сразу
    следом."""
    print("core-backup: учение архив развернуло, аудио не сверялось — %s"
          % причина, file=sys.stderr)


def проверка(target, пароль, root, имя=None, без_зеркала=None):
    ...
    `без_зеркала` — причина, по которой аудио сверять нечем (#63). Учение
    сверяет то, что зеркалили: зеркала не делали — аудио не проверяется, и
    это не поломка. Причина обязана быть строкой, а не `True`: молчание тут
    опаснее падения, поэтому в итог уходит `аудио_не_сверялось` с причиной, а
    не `аудио_сверено: 0` — ноль сверенных записей и «не сверяли вовсе»
    обязаны различаться в отчёте."""
...
def прогон(root, targets, пароль, keep, work, аудио=True, drill=True):
    ...
    if drill:
        ...
        # Учение идёт по первому носителю, где архив ещё на месте, а не по
        # `записано[0]`: список собран до зеркалирования, а носитель, который
        # отвалился после записи, унёс архив вместе с монтированием — на
        # точке монтирования остаётся пустой каталог на корневой ФС. Падать
        # «нет архивов ядра» на нём, имея живую копию у соседа, значит не
        # проверить ровно то, что как раз проверяемо. Не уцелело ни одной —
        # остаётся `записано[0]` и то же падение: ночь, от которой не
        # осталось ни одной читаемой копии, обязана быть красной.
        цель = next((t for t in записано
                     if os.path.exists(os.path.join(t, имя))), записано[0])
        # Учение сверяет то, что зеркалили (#63). Требовать аудио, которого
        # не зеркалили, — падение ночи из-за работы, которой не просили:
        # прежде каждая причина кончалась `RuntimeError: аудио … нет в
        # зеркале`. Причин три, и смотрим мы их у ОДНОГО носителя — того, по
        # которому пойдёт учение. Отвал соседней шары — штатная ночь
        # (`docs/backup-core.md`), и гасить из-за неё сверку на здоровом
        # носителе значило бы завести то самое зелёное молчание, против
        # которого весь этот код.
        #
        # 1. `--no-audio`: зеркала не делали вовсе. Причина боевая — флаг
        #    прописывают в крон.
        # 2. Носитель отвалился до зеркалирования: `зеркало_аудио` не звали
        #    вовсе (`пропущен`).
        # 3. Носитель отвалился посреди зеркалирования: часть файлов легла,
        #    хвост нет (`оборвано`). Физический факт тот же, что и у второй,
        #    разница лишь в том, на какой миллисекунде его заметили.
        #
        # Где вторая и третья срабатывают — важно, и довод тут ровно один.
        # После настоящего отвала точка монтирования пуста: файлов зеркала
        # нет, но нет и архива, а `цель` выбрана по архиву — значит учение
        # ушло к соседу и до подавления дело не дошло вовсе. Подавление
        # исполняется только при флапе, когда носитель вернулся к учению:
        # архив снова виден, зеркало неполно. И тогда промах учения не
        # случаен, а гарантирован порядком: зеркало идёт по `blobs` без
        # `order by`, свежайшие последними, а учение берёт `order by created
        # desc limit ПРОБА` — ровно хвост за точкой обрыва. Довод держится
        # на том, что `created` растёт вместе с rowid; бэкфилл задним числом
        # его сломает, но сломает в безопасную сторону — в лишнюю сверку, а
        # не в молчание. Правило же «не сверять то, чего не зеркалили» одно
        # на все три причины, и держать в коде три незачем.
        пропуск = зв.get(цель, {})
        без_зеркала = None
        if not аудио:
            без_зеркала = БЕЗ_ЗЕРКАЛА_ФЛАГ
        elif "пропущен" in пропуск:
            без_зеркала = "носитель не смонтирован при зеркалировании"
        elif "оборвано" in пропуск:
            без_зеркала = "зеркало оборвалось на середине"
        итог["проверка"] = проверка(цель, пароль, root, имя=имя,
                                    без_зеркала=без_зеркала)
        if без_зеркала:
            сказать_про_пропуск(без_зеркала)
...
def самопроверка():
    ...
    try:
        ...
        for кривой, слово in ((не_gpg, "не gpg"), (не_тот_файл, "не то аудио")):
            ...
            # И учение в этом итоге сверяло аудио, а не отговаривалось
            # причиной: зеркало на цели делали, сбой на соседнем носителе не
            # повод молчать. Ночь всё равно покраснеет ниже, но покраснеет с
            # правдой в отчёте.
            assert '"аудио_сверено"' in шум, (слово, шум[-400:])
        ...
        # А учение при этом сверило аудио по-настоящему. Это единственное
        # место, где отваливается НЕ тот носитель, по которому идёт учение, —
        # и значит единственное, где ловится обобщение причины на всех
        # («у кого-нибудь пропущено» вместо «у цели пропущено»). Без этой
        # строки отвал соседней шары — а он штатная ночь по
        # `docs/backup-core.md` — молча гасил бы сверку на здоровом носителе.
        assert r4["проверка"]["аудио_сверено"] == 1, r4
        ...
        # Свежая живая запись — не украшение сценария, а его условие. Учение
        # берёт три СВЕЖАЙШИЕ живые записи; к этому месту прежние блобы уже
        # вычищены прошлыми сценариями, сверять было бы нечего, и ночь без
        # правки не упала бы вовсе — застава сторожила бы имя ключа вместо
        # беды. С этой записью снятая ветка даёт настоящее
        # `RuntimeError: аудио … нет в зеркале`.
        тело7 = b"audio-number-seven"
        s7 = hashlib.sha256(тело7).hexdigest()
        p7 = mi.blob_path(root, s7, "wav")
        os.makedirs(os.path.dirname(p7), exist_ok=True)
        open(p7, "wb").write(тело7)
        con = mi.connect(root)
        con.execute("insert into blobs(sha256,path,bytes,mime,created)"
                    " values(?,?,?,?,?)",
                    (s7, p7, len(тело7), "audio", mi.now_iso()))
        con.close()

        # Вторая причина не сверять аудио, и `--no-audio` про неё ничего не
        # знает: отвалился носитель, по которому идёт учение. Зеркала на нём
        # нет по своей же защите (`пропущен`), а учение прежде требовало
        # оттуда аудио и роняло ночь целиком. Здесь каталог остаётся читаем —
        # `смонтирован` подменён, а файлы на месте; настоящий отвал уносит и
        # архив, и на нём стоит следующий сценарий.
        # Носители берём свои, а не прежние: сценарий стоит последним и не
        # имеет права сдвинуть чужие ночи, а `отвалился_после_архива`
        # опознаёт отвал по наличию архива — на прежних он уже лежит.
        головной = os.path.join(tmp, "target7")
        напарник = os.path.join(tmp, "target8")
        os.makedirs(головной, mode=0o700)
        os.makedirs(напарник, mode=0o700)

        def отвалился_головной(t, корень):
            if t == головной and glob.glob(
                    os.path.join(t, "core-2*.tar.gz.gpg")):
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = отвалился_головной
        перехват7 = io.StringIO()
        try:
            with contextlib.redirect_stderr(перехват7):
                r7 = прогон(**dict(общее, targets=[головной, напарник]))
        finally:
            mi.смонтирован = целый_смонтирован
        # Архив на него лёг — учение пойдёт именно по нему.
        assert r7["носители"][0] == головной, r7
        assert (r7["аудио"][головной]
                == {"пропущен": "носитель не смонтирован"}), r7
        # Главное: ночь не упала, а учение сказало, чего не делало.
        assert "аудио_сверено" not in r7["проверка"], r7
        assert (r7["проверка"]["аудио_не_сверялось"]
                == "носитель не смонтирован при зеркалировании"), r7
        # И сказало это не только в JSON: без строки в `stderr` пропуск
        # неотличим от штатной ночи для того, кто читает лог глазами.
        шум7 = перехват7.getvalue()
        assert "аудио не сверялось" in шум7, шум7[-400:]
        # А напарник до зеркалирования дошёл, и дошёл с работой: пропуск
        # головного не отменяет работы на здоровых носителях. Считаем именно
        # новые файлы — `"пропущен" not in ...` было бы тавтологией, подмена
        # `смонтирован` врёт только про головного и ложной эта проверка быть
        # не может ни при каком мутанте.
        assert r7["аудио"][напарник]["новых"] >= 1, r7

        # А теперь отвал, какой бывает на железе: каталог уезжает вместе с
        # монтированием, и архив, легший минуту назад, исчезает с ним. Учение
        # по `записано[0]` умирало тут раньше, чем доходило до аудио, —
        # `RuntimeError: нет архивов ядра` при живой копии у соседа.
        сорвавшийся = os.path.join(tmp, "target9")
        уцелевший = os.path.join(tmp, "target10")
        os.makedirs(сорвавшийся, mode=0o700)
        os.makedirs(уцелевший, mode=0o700)

        def отвал_с_каталогом(t, корень):
            if t == сорвавшийся and glob.glob(
                    os.path.join(t, "core-2*.tar.gz.gpg")):
                for p in glob.glob(os.path.join(t, "*")):
                    os.unlink(p)
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = отвал_с_каталогом
        try:
            with contextlib.redirect_stderr(io.StringIO()):
                r8 = прогон(**dict(общее, targets=[сорвавшийся, уцелевший]))
        finally:
            mi.смонтирован = целый_смонтирован
        # Архив лёг на оба, а к учению остался один — по нему и пошли.
        assert r8["носители"] == [сорвавшийся, уцелевший], r8
        assert r8["проверка"]["носитель"] == уцелевший, r8
        # И аудио на нём сверено по-настоящему: причины не сверять нет, а
        # молчаливое `аудио_не_сверялось` было бы тем самым зелёным, ради
        # различения которого #63 и заводили. Числом, а не фактом ключа: с
        # `in` застава прошла бы и на честном нуле, то есть перестала бы
        # замечать пропажу засева выше.
        assert r8["проверка"]["аудио_сверено"] == 1, r8

        # Третья причина — тот же физический факт, что вторая, только
        # замеченный на миллисекунду позже: носитель ушёл не до
        # зеркалирования, а посреди него. Часть файлов легла, хвост нет — а
        # учение берёт как раз хвост (`order by created desc` против обхода
        # по rowid), так что промах не случаен, а гарантирован. Прежде это
        # была та же ловушка: ночь краснела «нет в зеркале» из-за работы,
        # которой не просили. Ведём через `прогон` с учением: прямой вызов
        # `зеркало_аудио` (сценарий выше) этого пути не трогает вовсе.
        # Вторая живая запись: обрыв мерится не флагом, а фактом — носитель
        # жив, пока на него не лёг первый файл зеркала. С одной записью
        # зеркало кончается раньше, чем успевает оборваться, и сценарий
        # молча превращается в штатную ночь.
        тело9 = b"audio-number-nine"
        s9 = hashlib.sha256(тело9).hexdigest()
        p9 = mi.blob_path(root, s9, "wav")
        os.makedirs(os.path.dirname(p9), exist_ok=True)
        open(p9, "wb").write(тело9)
        con = mi.connect(root)
        con.execute("insert into blobs(sha256,path,bytes,mime,created)"
                    " values(?,?,?,?,?)",
                    (s9, p9, len(тело9), "audio", mi.now_iso()))
        con.close()

        оборвался = os.path.join(tmp, "target11")
        здоровый = os.path.join(tmp, "target12")
        os.makedirs(оборвался, mode=0o700)
        os.makedirs(здоровый, mode=0o700)

        def ушёл_посреди(t, корень):
            if t == оборвался and glob.glob(
                    os.path.join(t, "calls", "*", "*", "*.gpg")):
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = ушёл_посреди
        перехват9 = io.StringIO()
        try:
            with contextlib.redirect_stderr(перехват9):
                r9 = прогон(**dict(общее, targets=[оборвался, здоровый]))
        finally:
            mi.смонтирован = целый_смонтирован
        # Зеркало начато и брошено, архив на месте — учение идёт по нему же.
        assert (r9["аудио"][оборвался]["оборвано"]
                == "носитель не смонтирован"), r9
        assert r9["аудио"][оборвался]["новых"] == 1, r9
        assert r9["проверка"]["носитель"] == оборвался, r9
        # Ночь не упала, и причина у пропуска своя, третья: свалить обрыв в
        # «носитель не смонтирован при зеркалировании» значило бы сказать,
        # что зеркала не начинали, — а его начали и бросили.
        assert (r9["проверка"]["аудио_не_сверялось"]
                == "зеркало оборвалось на середине"), r9
        шум9 = перехват9.getvalue()
        assert "зеркало оборвалось на середине" in шум9, шум9[-400:]

        # Фолбэк на `записано[0]`. Учение идёт по носителю с уцелевшим
        # архивом, но уцелеть может ни одного — тогда ночь обязана падать, и
        # падать ВНЯТНО. Без дефолта у `next` то же место кончается голым
        # `StopIteration` без единого адреса в тексте: разбираться по такому
        # логу не с чего. Ночь с двумя носителями, потерявшими архив оба.
        пропалА = os.path.join(tmp, "target13")
        пропалБ = os.path.join(tmp, "target14")
        os.makedirs(пропалА, mode=0o700)
        os.makedirs(пропалБ, mode=0o700)

        def унесло_оба(t, корень):
            if t in (пропалА, пропалБ) and glob.glob(
                    os.path.join(t, "core-2*.tar.gz.gpg")):
                for п in glob.glob(os.path.join(t, "*")):
                    os.unlink(п)
                return False
            return целый_смонтирован(t, корень)

        mi.смонтирован = унесло_оба
        перехват10 = io.StringIO()
        try:
            with contextlib.redirect_stderr(перехват10):
                прогон(**dict(общее, targets=[пропалА, пропалБ]))
            raise AssertionError("ночь без единой копии архива прошла молча")
        except RuntimeError as e:
            assert "нет архивов ядра" in str(e), str(e)
            # И носитель назван: без имени в тексте фолбэк не отличить от
            # падения на пустом списке.
            assert пропалА in str(e), str(e)
        finally:
            mi.смонтирован = целый_смонтирован
        # Причина не объявлена: печатать её до учения значило бы обещать в
        # логе развёрнутый архив за строку до «нет архивов ядра». Порядок
        # вызовов держится этой строкой и только ею — а сама она держится за
        # `аудио не сверялось`, кусок текста, закреплённый положительно и
        # выше по самопроверке, и в `tests/test_backup_flags.py`. За
        # `архив развернуло` держаться нельзя: его не закрепляет ничто, и
        # переписанная формулировка сняла бы заставу молча.
        assert "аудио не сверялось" not in перехват10.getvalue(), \
            перехват10.getvalue()[-400:]
...
def main():
    ...
    if a.drill_only:
        ...
        # `--no-audio` осмыслен и здесь: ручное учение после ночи, которая
        # шла без зеркала, иначе упало бы тем же «нет в зеркале» (#63).
        причина = БЕЗ_ЗЕРКАЛА_ФЛАГ if a.no_audio else None
        r = проверка(targets[0], a.pass_file, a.root, без_зеркала=причина)
        if причина:
