...
def self_check():
    ...
    # лишь с третьего раза: `env()` НЕ стрипает. Из окружения значение
    # берётся как есть (`:57`), а в файловой ветке кавычки снимаются
    # после пробелов (`:65`), так что ` 123456789 ` внутри кавычек
    # доезжает сюда целиком. В бою крайние пробелы у незакавыченного
    # значения режет systemd (`man systemd.exec`, EnvironmentFile=) —
    # там мутант «снять strip» безвреден, но не здесь и не при кавычках.
