...
if RAW_DAYS < 0:
    ...
    # Правило одно: есть ближайшее допустимое значение — берём его, нет —
    # задокументированный дефолт ТЗ §17. У «больше ста лет» ближайшее и есть
    # потолок; у «тридцать» и у минуса ближайшего нет вовсе, брать нечего.
    # Больше ветки ничем не различаются: снести сырьё, которое владелец просил
    # сохранить, в ближайшие 4:40 способна любая из трёх — ревьюер показал это
    # прогоном в круге 2. Цена ошибки у них одна, разный только откат.
