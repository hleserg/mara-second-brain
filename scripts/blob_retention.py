...
if RAW_DAYS < 0:
    ...
    # отнимает: файлов старше ста лет на диске нет, значит прогон с потолком
    # удаляет ровно то же, что удалил бы миллион суток, если бы досчитал.
    ...
    # числа нет.
    # Дробная запись (`45.0`, `1e6`) сюда не доходит вовсе: `int` её не
    # берёт, и она уезжает веткой выше, в «беру 30», то есть в подмену
    # смысла. Это известный предел, а не намерение: чинится `int(float(x))`
    # под `except (ValueError, OverflowError)`, отдельной правкой —
    # docs/decisions.md. Нашёл ревьюер, круг 3.
    # Ценой ошибки ветки не различаются: снести сырьё, которое владелец просил
    # сохранить, в ближайшие 4:40 способна любая из трёх — ревьюер показал это
    # прогоном в круге 2.
