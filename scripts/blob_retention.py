...
if RAW_DAYS < 0:
    ...
    # все значения означают одно и то же — «не убирать», — и кламп на него не
    # меняет ничего: сто лет и миллион суток дают одинаковый прогон. Ниже так
    # не выходит: у «тридцать» подставлять нечего вовсе, а у минуса ближайшее
    # допустимое — ноль, то есть «держать только сегодняшний день», и это уже
    # другой смысл, а не тот же. Там остаётся дефолт ТЗ §17.
