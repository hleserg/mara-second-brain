...
if math.isnan(СРОК):
    ...
    RAW_DAYS = 30
elif math.copysign(1, СРОК) < 0:
    # Знак, а не сравнение с нулём: `-0.0 < 0` — ложь, а минус там есть, и
    # `ceil(-0.0)` даёт ноль, то есть «держать только сегодняшний день».
    # Пропустив его молча, застава сносила бы всё сырьё ровно там, где
    # поставлена этого не допустить. Той же записью ловятся `-0`, `-0e10` и
    # `-1e-400`; `-0` до этой правки давал молчаливый ноль и через `int`.
    # Нашёл ревьюер, круг 1.
    # Отрицательный срок — та же опечатка владельца, но тише и дороже:
    # разбор её пропускает, а `raw_sweep` уносит порог в будущее и удаляет
