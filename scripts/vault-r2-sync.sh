...
# Жёсткий потолок: флок общий с автокоммитом, и пока R2 отвергает перезапись
# существующих ключей (500 InternalError), прогон гарантированно упирается в
# него весь отведённый срок. Две минуты раз в десять — не голодает git-крон.
exec timeout -k 10s 2m "$RCLONE" "${args[@]}"
