...
# Правка config/r2-filters.txt тоже требует --resync: rclone запоминает состав
# фильтров и после их смены отказывается работать («filters file has changed»),
# а сама смена состояния не сносит — значит, синк встанет молча до тех пор,
# пока кто-нибудь не уберёт .lst из ~/.cache/rclone/bisync и не прогонит
# скрипт вручную, надзорно. Проверка ниже при этом отработает как надо.
#
