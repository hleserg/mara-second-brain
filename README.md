...
## Известная поломка: 14 «отравленных» объектов в R2

С ночи 1 сентября часть ключей в бакете `obsidian-vault` не перезаписывается и
не удаляется: `PutObject`/`DeleteObject` отдают `HTTP 500 InternalError` после
всех ретраев, поодиночке и пакетом одинаково.
...
| Свежий бакет: 40 объектов, перезапись через 5 мин, удаление | 0 ошибок |

Значит порча привязана к конкретным объектам, детерминирована (ретраи не
помогут) и локальна для этого бакета — новый бакет здоров. Отличить
испорченный объект заранее нельзя: у здорового `copilot/data.json` и
испорченного `kb/howto/atman/ENDPOINTS.md` метаданные одинаковые.

Лечение сейчас — исключить их из синка (`config/r2-filters.txt`): 14 файлов,
которые есть в волте, плюс 14 «сирот», которые остались только в R2. После
этого `--resync` проходит за 29 секунд, остальные 170 файлов ездят нормально.
Цена: по этим 14 заметкам правки не доедут до телефона, в R2 по ним лежит
устаревшая копия. Волт и git не страдают — они источник истины (§1).

Радикальное лечение — переезд на новый бакет; требует перенастроить Remotely
Save на телефоне, маке и винде одновременно, поэтому только по решению
владельца.

**Не гонять `scripts/stage0-selftest.sh`, пока это не починено.** Он кладёт
