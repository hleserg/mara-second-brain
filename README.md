...
## Известная поломка: четырнадцать «отравленных» объектов в R2

С ночи 1 сентября четырнадцать конкретных ключей в бакете не удаляются и не
перезаписываются: `PutObject` и `DeleteObject` по ним отдают `HTTP 500` либо
висят до таймаута. Список — в `config/r2-orphans.txt`.

**Сам бакет исправен.** Проверено контрольным опытом: создать новый ключ,
перезаписать его, удалить — всё проходит; то же самое на давно лежавшем
`.obsidian/plugins/copilot/data.json` (2026-08-20) прошло без ошибок. То есть
синк с телефоном работает полностью, включая правки существующих заметок, —
кроме этих четырнадцати файлов, которых в волте всё равно уже нет.

Они занесены в `config/r2-filters.txt`, чтобы bisync об них не спотыкался,
`scripts/r2-retry-orphans.sh` периодически пробует их снова. Когда Cloudflare
их отпустит — убрать строки из фильтра и сделать `--resync`.
