...
- **Human owner — final authority:** одобряет значимые архитектурные решения, опасные миграции и privacy changes. Право merge владелец делегировал (см. поправку 2026-09-03 ниже).
...
### 14.2a. Поправка владельца 2026-09-03: merge

Владелец передал право merge Claude Code. Дословно: «мердж по готовности
самостоятельно без моего решения» и «Если недоступен ревью клода — ревьювит
субагент твой. Максимально — 5 кругов ревью, дальше мердж в мэйн за исключением
случаев когда из-за мерджа может что-то сломаться в мэйне».

Что считается готовностью — все пункты сразу:

- CI зелёный;
- `scripts/run-tests.sh` зелёный на **смердженном** дереве, а не только на ветке;
- ревью завершено: Codex, если доступен, иначе субагент;
- кругов ревью не больше пяти; после пятого мердж, а оставшиеся замечания — в issue.

Не мерджим, даже когда круги исчерпаны:

- гейт красный;
- конфликт не разрешён;
- мердж ломает живого потребителя `main` — на doctor и маке работают демоны с
  этого кода.

**Поправка владельца 2026-09-11.** Право merge не есть право деплоя — так было
до этой даты, и выкат оставался владельцу. С 2026-09-11 выкат на doctor передан
агенту (ответ на мешок М5 п.1 в `docs/PLAN-to-done.md`); порядок исполнения —
`docs/RUNBOOK-deploy.md`. Передача касается только выката кода: **гейт Г4
остаётся** — миграция схемы реестра требует отдельного разрешения, свежей
резервной копии и пройденного учения восстановления. Осторожно с формулировкой
«миграция — отдельная операция»: в этом коде миграции реестра выполняются из
`mara_ingest.connect()` — проверка схемы идёт при каждом открытии базы,
перестройка выполняется один раз (`_ужать_ledger` выходит сразу, если ключи уже
`not null`). То есть выкат кода и есть выкат миграции, и выполняет её первый,
кто откроет базу: рестарт демона или ближайший крон. Шаг 2 ранбука проверяет
это до сброса дерева.

Оговорка про деплой не риторическая: на doctor `git pull` не делает ни один
cron, checkout там обновляется только ранбуком.
...
- [x] Работа идёт PR-only. *(2026-09-11: защита `main` включена, прямой пуш запрещён и для админов — `enforce_admins: true`.)*
- [ ] Каждый data model/migration/security/recorder PR получил независимый review Codex.
- [x] Максимум три субагента по умолчанию и budget guardrails зафиксированы в contributor/agent docs. *(`docs/CONTRIBUTING.md`, раздел «Ограничения агентов».)*
