...
if ! out=$(crontab -l 2>&1); then
  case "$out" in
    # `out` тут держит текст отказа, и без обнуления «крона нет» ниже
    # прочтётся как «крон был непустой»: на чистой машине откат не сработает
    *"no crontab for"*|"") out="" ;;
...
if diff -u <(блок) <(рендер) > "$verify"; then
  ...
  if [ -s "$backup" ] || [ "$WAS_EMPTY" = 1 ]; then
    ...
    # без `||` отказ здесь убил бы скрипт по `set -e` молча, на последнем
    # шаге, который ещё мог спасти расписание: ни строки в лог, ни выхода 1
    crontab "$backup" || echo "== откат не удался: crontab остался таким," \
         "каким его оставила сорванная установка, копия в $backup" >&2
  else
