...
class События(unittest.TestCase):
    ...
    def test_повтор_считается_по_нормализованному_тексту(self):
        """«ок  ок» и «ок ок» дают один ключ, значит и в счёт идут как одно:
        счёт по сырому тексту оставил бы им общий source_id."""
        msgs = w.parse(["02.09.26, 14:05 - Анна: ок  ок", "02.09.26, 14:05 - Анна: ок ок"], tz=МСК)
        ев = list(w.events(msgs, "Анна"))
        self.assertNotEqual(ев[0]["source_id"], ев[1]["source_id"])
