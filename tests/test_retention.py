...
class Сырьё(unittest.TestCase):
    ...
    def test_плохой_MARA_RAW_DAYS_не_роняет_уборку_в_4_40(self):
        ...
        застава берёт любой минус, и сужение до `< -1` на тридцатке было бы
        ...
        потерю, против которой застава стоит, только молча. `nan` же не
        берёт ни застава знака, ни потолок: `copysign` у него плюс, а
        `nan > ПОТОЛОК` ложно, — и без заставы `isnan` он доехал бы до
        `ceil(nan)`, а тот бросает уже на импорте: крон в 4:40 снова отдаёт
        `EXIT=1` и не убирает ничего. У `-nan` знак минус, и застава знака
        его взяла бы, но до неё он тоже не доезжает — `isnan` стоит первой.
        Нашёл ревьюер, круг 5.
