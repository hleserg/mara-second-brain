...
class Прогон(unittest.TestCase):
    ...
    def test_отказ_уборки_не_валит_прогон(self):
        ...
        цель = os.path.join(self.tmp, "target")
        второй = os.path.join(self.tmp, "target2")
        # Старые архивы на носителе, куда сегодня не записалось: ротация не
        # имеет права их трогать. Носитель, потерявший связь, живёт последней
        # удачной копией — вычистить её по `keep` значило бы оставить его ни
        # с чем. Заодно это единственный вход, отличающий ротацию по
        # `записано` от ротации по `targets`.
        os.makedirs(второй)
        ...
        self.assertEqual(len(glob.glob(os.path.join(второй, "core-*"))), 3,
                         "ротация выкосила архивы носителя, куда не писали")
