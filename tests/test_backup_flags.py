...
class Флаги(unittest.TestCase):
    ...
    def test_drill_only_разворачивает_боевой_архив(self):
        ...
        # машине три: сайдкар рядом с архивом (круг 1: заглушка читала его и
        # прошла весь гейт), живая база (круг 2: заглушка считала по ней — и
        # тоже прошла) и зеркало аудио на носителе (круг 3). Сайдкар портим,
        # живую базу разводим лишней строкой в `events` — таблице, которой
        # нет ни в зеркале, ни в блобах, так что ни счётом файлов зеркала, ни
        # отсевом мёртвых строк её не восстановить, — правду держим снимком
        # в памяти теста. Круги 1 и 2 разводили `blobs`, и это ровно та
        # ошибка: столбец, который заглушка умеет пересчитать из другого
        # источника, оракулом не становится.
        #
        ...
        живая.execute("insert into events(id, kind, received) values(?,?,?)",
                      ("разводка", "проверка", mi.now_iso()))
        живая.commit()
        живая.close()
