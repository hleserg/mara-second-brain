...
class Флаги(unittest.TestCase):
    ...
    def test_drill_only_разворачивает_боевой_архив(self):
        ...
        self.assertEqual(открыт.returncode, 0,
                         ...
                         + открыт.stderr)
        # Оракул обязан знать правду отдельно от кода под тестом, а сайдкар
        # рядом с архивом этого не даёт: до него дотягивается и заглушка вместо
        # `проверка(...)` — круг 1 такую перегнал, и она прошла весь гейт.
        # Поэтому сайдкар портим, а правду держим снимком: настоящая `проверка`
        # считает счётчики по базе **внутри** архива и манифесту из тарбола
        # (`core-backup.py:291-292`), сайдкара не читает вовсе и порчи не
        # замечает; заглушка, которая счёт берёт с носителя, отдаёт 999.
        ...
        # Заодно и `--root`: на стенде ровно один блоб, у боевого корня — не один.
