...
class Флаги(unittest.TestCase):
    ...
    def test_drill_only_разворачивает_боевой_архив(self):
        ...
        # неотличима от настоящего звонка ни одним столбцом, и любой отсев по
        # форме снесёт вместе с ней боевые события.
        #
        ...
        было = сайдкар["counts"]
        # Портим ВСЕ девять счётчиков, а не один `blobs`: круг 4 показал, что
        # испорченное поле — это ровно одно поле, а восемь остальных сайдкар
        # отдавал целыми. Заглушка «`events` из сайдкара, `blobs` счётом
        # файлов зеркала» собирала полную правду из двух достижимых копий и
        # проходила **весь гейт** (`Ran 392`, `OK`), не вызвав ни gpg, ни
        # tarfile. Порча одного поля — не порча источника.
        json.dump(dict(сайдкар, counts={k: 999 for k in было}),
