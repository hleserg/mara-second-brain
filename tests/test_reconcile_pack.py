...
class ВозрастПакета(unittest.TestCase):
    ...
    def test_опечатка_в_MARA_RAW_DAYS_докладывается_отдельной_находкой(self):
        ...
        # Тождество, а не вхождение подстроки: оба токена («тридцать» и имя
        # переменной) достижимы из `os.environ`, и сверка, собравшая текст
        # сама, прошла бы оба `assertIn` зелёной. А тезис правки ровно
        # обратный — причину кладёт тот, кто её знает. Свежий модуль после
        # `rc.run` лежит в `sys.modules`; глобальный `br` в этом файле указывал
        # бы на объект, импортированный до подмены. Нашёл ревьюер, круг 1.
        self.assertEqual(sys.modules["blob_retention"].ОШИБКА_КОНФИГА,
                         находки["ретеншен-конфиг"]["detail"])
