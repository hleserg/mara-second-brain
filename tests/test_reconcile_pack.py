...
class ВозрастПакета(unittest.TestCase):
    ...
    def test_опечатка_в_MARA_RAW_DAYS_докладывается_отдельной_находкой(self):
        ...
        self.addCleanup(sys.modules.__setitem__, "blob_retention",
                        blob_retention)
        # Значений пять, и второе здесь не про минус, а про оракул: сверка,
        ...
        # владелец, поправив кавычки, получал бы ту же жалобу снова. Пятое
        # — дробь ниже суток: оба оракула дроби в `test_retention` стоят выше
        # единицы, и мутант, гасящий жалобу при `СРОК <= 1`, проходил полный
        # гейт зелёным — `MARA_RAW_DAYS=0.5` при нём брал сутки молча, и
        # владелец, промахнувшийся мимо `30`, узнавал об этом только по
        # пропавшему сырью. Нашёл ревьюер, круг 2.
        #
        # Число в куске текста, а не одна причина: мутант, усекающий дробь
        # ниже суток вниз, причину оставляет прежней и даёт «беру 0» — то
        # есть порог на сегодня, и вчерашние логи уходят. Вверх, а не вниз —
        # заглавное решение этого PR, и ниже единицы его не закреплял никто.
        # Нашёл ревьюер, круг 3.
        for значение, кусок in (("тридцать", "не число"),
                                ("-1", "отрицательный срок"),
                                ("1000000", "больше ста лет"),
                                ("44.5", "дробный срок"),
                                ("0.5", "дробный срок, беру 1")):
