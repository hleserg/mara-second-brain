...
class ТестАдресовВРепозитории(unittest.TestCase):
    ...
    # 192.168/10/172.16 проверять там нечего. vault-cleanup-step3.md убран по
    # ревью: за исключением он прятал настоящий адрес стенда, а «отчёт о старой
    # чистке» — ровно то место, где такое и оседает
    ...
    def test_сторож_видит_tailnet(self):
