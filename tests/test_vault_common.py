...
class ТестАдресовВРепозитории(unittest.TestCase):
    ...
    # октета с `(?!/)` на хвосте, потому что запись диапазона `100.64.0.0/10`
    # в описании правила pf — не адрес хоста, и подставлять вместо неё
    # плейсхолдер значило бы врать про правило. У RFC1918 такой поблажки нет
    # намеренно: там литерал сети в дереве не нужен ни разу.
    СЕТЬ = re.compile(
    ...
    def test_сторож_видит_tailnet_и_пропускает_его_диапазон(self):
        ...
        # запись диапазона в описании правила pf — не адрес хоста
        self.assertIsNone(self.СЕТЬ.search("правило NAT для 100.64.0.0/10"))
        ...
        # у RFC1918 поблажки на диапазон нет: там литерал не нужен вовсе
        self.assertTrue(self.СЕТЬ.search("192.168.0.0/16"))
