...
class ТестРазбораEnv(unittest.TestCase):
    ...
    # Каждая строка — случай, на котором самодельный разбор расходился с шеллом
    # или мог бы разойтись. Ключи только ASCII: имена переменных придётся
    # подставлять в bash, а Cyrillic он в имени не примет.
    ...
    def test_разбор_совпадает_с_шеллом(self):
        ...
        же файл значит для второго его читателя. Разошлись они дважды, и оба
        раза тихо, поэтому спрашиваем теперь напрямую.
        """
        ключи = ["K%d" % i for i in range(1, 8)]
        путь = self.файл(self.ОРАКУЛ)
        печать = " ".join('"$%s"' % k for k in ключи)
        r = subprocess.run(["bash", "-c",
                            'set -a; . "$1"; set +a; printf "%s\\0" ' + печать,
                            "bash", путь],
                           capture_output=True, check=True)
        шелл = dict(zip(ключи, r.stdout.decode().split("\0")))
    ...
    def test_незакрытая_кавычка_уносит_только_свою_строку(self):
        """`shlex` на такой строке бросает ValueError. Ловим построчно: ключи
        после битой строки нужны не меньше, чем до неё."""
