...
class ТестПоследнийДайджест(unittest.TestCase):
    ...
    **безусловно**, каким бы ни был исход отправки
    ...
    def test_доставленный_отдаётся_как_прежде(self):
        ...
        #
        # Доставленных две, и с разным временем: на одной `order by sent_at
        # desc` неотличим от `asc` — правка трогает ту же строку запроса,
        # и порядок обязан быть под оракулом вместе с фильтром.
        self.дайджест("d0", "sent", 8, "вчерашний, тоже доставленный")
        eid = self.дайджест("d1", "sent", 9, "свежий и доставленный")
        b = contextd.bootstrap(self.con)
