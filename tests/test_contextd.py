...
class ТестПоследнийДайджест(unittest.TestCase):
    ...
    **безусловно**, каким бы ни был исход отправки
    ...
    §8.3, #61). Проверяются все три: фильтр по одному имени состояния
    прошёл бы тест на любом из остальных.
    """
    ...
    def дайджест(self, ключ, state, час, text):
        eid, _ = mi.put_event(self.con, {"kind": "call", "source": "phone",
                                         "source_id": ключ,
                                         "payload": {"text": "x"}})
        self.con.execute(
    ...
    def test_доставленный_отдаётся_как_прежде(self):
        ...
        #
        # Доставленных две, и врозь по трём осям сразу. Свежая по
        # `sent_at` вставлена **первой** (меньший `rowid`) и с лексически
        # меньшим `id`. Иначе `sent_at`, `id` и `rowid` растут вместе, а
        # тогда оракул держит только направление сортировки, но не столбец:
        # `order by id desc` и `order by rowid desc` проходили весь гейт
        # (перегнано: оба выживали). В бою `digests.id` — `uuid4`
        # (`call_digest.py:160`), то есть сортировка по нему отдавала бы
        # владельцу произвольный доставленный дайджест.
        #
        # Четвёртая ось, `event_id`, фикстурой не берётся вовсе: он тоже
        # `uuid4` (`mara_ingest.py:174`), и `order by event_id desc` — монета.
        # Такой мутант не мерка ни в ту, ни в другую сторону.
        eid = self.дайджест("a1", "sent", 9, "свежий и доставленный")
        ...
        self.assertEqual(b["last_digest"]["text"], "свежий и доставленный",
                         "последний — самый свежий по sent_at, а не первый "
                         "по id, по rowid или по порядку вставки")
