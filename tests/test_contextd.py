...
class ТестПоследнийДайджест(unittest.TestCase):
    """`bootstrap` обещает клиенту «последний дайджест», а отдаёт последний
    записанный (#65). Разница не косметическая: `sent_at` пишется
    **безусловно**, каким бы ни был исход отправки
    (`call_digest.py:163-166` — `mi.now_iso()` идёт в ту же строку, что и
    `state`), а `state` в выборку не входит и клиенту не приходит вовсе.
    Владелец видел бы как «последний отправленный» текст, которого не
    получал, и отличить не мог бы ничем.

    Недоставленных состояний три: `failed` (сбой сети), `no-transport`
    (нет токена или адресата) и `not-private` (адресат не личный чат,
    §8.3 `TZ.md`, #61). Проверяются все три: фильтр по одному имени
    состояния прошёл бы тест на любом из остальных.
    """

    def setUp(self):
        каталог = tempfile.mkdtemp()
        mi.ROOT = каталог
        self.con = mi.connect(каталог)
        self.addCleanup(self.con.close)

    def событие(self, ключ):
        eid, _ = mi.put_event(self.con, {"kind": "call", "source": "phone",
                                         "source_id": ключ,
                                         "payload": {"text": "x"}})
        return eid

    def дайджест(self, ключ, state, час, text, eid=None):
        eid = eid or self.событие(ключ)
        self.con.execute(
            "insert into digests(id,event_id,chat_id,text,items_json,"
            "sent_at,state) values(?,?,?,?,?,?,?)",
            (ключ, eid, "123456789", text, "[]",
             "2026-09-06T%02d:00:00+03:00" % час, state))
        return eid

    def test_недоставленный_не_выдаётся_за_последний_отправленный(self):
        # Строк две, и недоставленная **позже**: на одной строке фильтр
        # неотличим от его отсутствия, а на двух с обратным порядком —
        # от `order by` в другую сторону.
        self.дайджест("d0", "sent", 8, "этот владелец получил")
        for i, состояние in enumerate(("failed", "no-transport",
                                       "not-private")):
            self.дайджест("d%d" % (i + 1), состояние, 9 + i,
                          "этот не уехал: " + состояние)
            b = contextd.bootstrap(self.con)
            self.assertEqual(
                b["last_digest"]["text"], "этот владелец получил",
                "%s: клиенту показан текст, которого владелец не получал"
                % состояние)

    def test_когда_не_уехало_ничего_последнего_дайджеста_нет(self):
        self.дайджест("d1", "no-transport", 9, "не уехал")
        b = contextd.bootstrap(self.con)
        self.assertIsNone(b["last_digest"],
                          "«дайджеста ещё нет» — правда, а «вот последний, "
                          "и он недоставлен» клиенту не выразить: `state` "
                          "в ответе нет")

    def test_доставленный_отдаётся_как_прежде(self):
        # Застава от починки, которая гасит ответ целиком: фильтр обязан
        # убирать недоставленное, а не всё подряд.
        #
        # Доставленных три, и свежая по `sent_at` не крайняя больше ни по
        # одной оси, по которой этот запрос мог бы сортироваться: вставлена
        # второй (средний `rowid`), с лексически средним `id` и средним из
        # трёх настоящих `event_id`. Иначе оракул держит направление
        # сортировки, но не столбец. Перегнано своей песочницей:
        # на симметричной фикстуре, где `sent_at`, `id` и `rowid` росли
        # вместе, `order by id desc` и `order by rowid desc` выживали весь
        # набор песочницы (база `Ran 125, OK`); на двух строках держались и
        # они, и направление, но `order by event_id desc` краснел 9 заходов
        # из 20 — монета, то есть ось не взята, а вовсе снятый `order by` не
        # краснел ни разу из 20: без сортировки SQLite отдаёт скан по
        # `rowid`, а там первой лежала как раз свежая.
        #
        # В бою и `digests.id` (`call_digest.py:159`), и `event_id`
        # (`mara_ingest.py:174`) — `uuid4`: сортировка по любому из них
        # отдавала бы владельцу произвольный доставленный дайджест.
        события = sorted(self.событие("e%d" % i) for i in range(3))
        self.дайджест("a1", "sent", 7, "позавчерашний, тоже доставленный",
                      события[0])
        eid = self.дайджест("m1", "sent", 9, "свежий и доставленный",
                            события[1])
        self.дайджест("z0", "sent", 8, "вчерашний, тоже доставленный",
                      события[2])
        b = contextd.bootstrap(self.con)
        self.assertEqual(b["last_digest"]["event_id"], eid)
        self.assertEqual(b["last_digest"]["text"], "свежий и доставленный",
                         "последний — самый свежий по sent_at, а не крайний "
                         "по id, по event_id, по rowid или по порядку "
                         "вставки")
...
class ТестПределЗаливок(unittest.TestCase):
    ...
    def дождаться(self, сколько, жалоба):
        ...
        Под замком, потому что словарь правит тот самый поток: чтение на
        живом `_заливки` однажды поймает `dictionary changed size` и станет
        мигающим тестом — дороже одной строки.
        """
        предел = time.monotonic() + 10
        while time.monotonic() < предел:
            with contextd._замок:
                сумма = sum(contextd._заливки.values())
            if сумма == сколько:
                return
            time.sleep(0.02)
        with contextd._замок:
            снимок = dict(contextd._заливки)
        self.fail("%s: %r" % (жалоба, снимок))
    ...
    def test_вторая_заливка_с_того_же_устройства_получает_503(self):
        ...
        try:
            ...
            # Предел на устройство, а не на сервер — то есть ровно та
            # формулировка, которая стоит в #39. Без этой строки предел,
            # ставший глобальным, пережил бы весь гейт: проверено мутантом,
            # заменяющим ключ на константу.
            self.assertEqual(self.залить(срв, токен2, eid_в, в), 200)
            держим.close()
