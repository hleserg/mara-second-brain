...
class Сквозной(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        os.environ.update({
            ...
            # Дата звонка здесь прибита (карточка разговора проверяется по
            # пути `2026-09-02-1405-anna.md`), поэтому к любому реальному
            # «сейчас» он старее суток. Застава свежести из call_digest.py
            # молчала бы тут навсегда и глушила бы собственного свидетеля
            # доставки; её саму проверяет `Свежесть` в test_call_digest.
            "MARA_DIGEST_MAX_AGE_H": "1000000",
        })
